...
{
    //xx crypto_utils.privateDecrypt_long(buf,this.privateKey,128);
    return crypto_utils.privateDecrypt_long(buffer,privateKey,128,crypto_utils.RSA_PKCS1_PADDING);
}
function RSAOAEP_Decrypt(buffer,privateKey)
{
    return crypto_utils.privateDecrypt(buffer,privateKey,256,crypto_utils.RSA_PKCS1_OAEP_PADDING);
}
// --------------------
...
function RSAPKCS1V15SHA1_Verify(buffer,certificate) {
    ...
    var signatureIsOK =  crypto_utils.verifyChunkSignature(buffer,options);
...
{
    return crypto_utils.publicEncrypt_long(buffer, publicKey,128,11,crypto_utils.RSA_PKCS1_PADDING);
    //xcx return crypto_utils.publicEncrypt(buffer,publicKey,crypto_utils.RSA_PKCS1_PADDING);
}

function RSAOAEP_Encrypt(buffer,publicKey)
{
    assert(false);
    return crypto_utils.publicEncrypt(buffer,publicKey,crypto_utils.RSA_PKCS1_OAEP_PADDING);
}
...
function compute_derived_keys(serverNonce,clientNonce) {
    ...
    if (clientNonce && serverNonce) {
        var options = {
            ...
            encryptingBlockSize: self.symmetricKeyLength,
...
function getCryptoFactory(securityPolicy) {
    ...
    switch(securityPolicy.key) {
        ...
        case SecurityPolicy.Basic128Rsa15.key:
            ...
            data.cipherBlockSize = 128;
            data.plainBlockSize  = 128-11;
