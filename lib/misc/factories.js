...
var _defaultType = [
    ...
    { name: "DateTime",encode: ec.encodeDateTime, decode: ec.decodeDateTime, defaultValue: exports.minDate },
    { name: "Guid",    encode: ec.encodeGuid,     decode: ec.encodeGuid,     defaultValue: defaultGuid },

    { name: "ByteString",encode: ec.encodeByteString, decode: ec.decodeByteString, defaultValue: function () {return new Buffer(0); }},
    { name: "XmlElement",encode: ec.encodeXmlElement, decode: ec.decodeXmlElement,     defaultValue: defaultXmlElement },
...
function registerType(name, encodeFunc, decodeFunc, defaultValue,subType) {
    assert(_.isFunction(encodeFunc));
    assert(_.isFunction(decodeFunc));
    assert(typeof name === "string");
    //xx console.log("registerType " +  name);
    var obj = {
        name: name,
        encode: encodeFunc,
        decode: decodeFunc,
...
function BaseObject(options) {
    ...
    // it is time to ask the scheme construct_hook method,if any, to tweak the option objet.
...
BaseObject.prototype.toJSON = function () {
    ...
    if (this._schema.toString) {
        ...
        return Object.toJSON.apply(this,arguments);
    }
};
...
function registerBasicType(schema) {
    ...
    registerType(name, t.encode, t.decode, t.defaultValue,schema.subtype);
    return null;
}
...
function registerObject(schema) {
    ...
    if (schema.hasOwnProperty("isEnum")) {
        assert(0 && " DO NOT USE registerObject to register an Enumeration type anymore, use  registerEnumeration instead!!!");
        return registerEnumeration(schema);
    } else if (schema.hasOwnProperty("subtype")) {
        assert(0 && " DO NOT USE registerObject to register a basic type anymore !!!");
        return registerBasicType(schema);
    } else {
        return registerComplexType(schema);
    }
}
