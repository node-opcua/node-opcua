...
var _defaultType = [
    ...
    {   name: "Any",
        encode: function(){assert(false,"type 'Any' cannot be encoded");},
        decode: function(){assert(false,"type 'Any' cannot be decoded");}
    },
    { name: "Boolean", encode: ec.encodeBoolean,  decode: ec.decodeBoolean,  defaultValue: false, coerce: ec.coerceBoolean },
    ...
    { name: "Int64",   encode: ec.encodeInt64,    decode: ec.decodeInt64,    defaultValue: 0 ,  coerce: ec.coerceInt64 },
    { name: "UInt64",  encode: ec.encodeUInt64,   decode: ec.decodeUInt64,   defaultValue: 0 ,  coerce: ec.coerceUInt64  },
...
function registerType(schema) {
    ...
    _defaultTypeMap[schema.name] = schema;
}
...
function _decode_by_type(obj, fieldType, stream) {
    var _t = _defaultTypeMap[fieldType];
    if ((_t["decode"] === undefined ) || (!_t.decode instanceof Function)) {
...
function _decode_(obj, objDesc, stream, options) {
    ...
    var fields=objDesc.fields;
    var n_field=fields.length;
    var i_field,field;
    var member,fieldType;


    for (i_field=0;i_field<n_field;i_field++) {  field = fields[i_field];
...
var objectVisitor = function (self, schema, data, callback) {
    ...
    for( i =0; i< n ; i++ ) {
        ...
        var e = _enumerations[fieldType];
        if (e) {
            var typedEnum = e.typedEnum;
            callback("enumeration", self, field, typedEnum, data);

        } else {
            var f = factories[fieldType];
            if (f) {
                callback("complex", self, field, null, data);
            } else {
                var d  = _defaultTypeMap[fieldType];
                if (d) {
...
function BaseObject(options) {
    ...
    _visit_schema_chain(self,schema,options,perform_construction);
...
BaseObject.prototype.encode = function(stream) {
    ...
    _visit_schema_chain(self,schema,stream,_encode);
};

function _decode(schema, stream, options) {
    var self = this;
    if (schema.decode) {
...
BaseObject.prototype.decode = function (stream, options) {
    ...
    _visit_schema_chain(self,schema,stream,_decode, options);
};
