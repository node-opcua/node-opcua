...
function _array_ellypsis(value) {
    if (!value) {
        ...
        //xxassert(_.isArray(value));
        var v = [];
        var m  =Math.min(10, value.length);
        for (var i = 0; i <m ; i++) {
            var element = value[i];
...
var _exploreObject = function (self, field,  data) {
    ...
    var _dump_simple_value = function(self,field,data,value,fieldType) {
        ...
        if (value instanceof Buffer) {
            ...
            if (field.isArray) {
                str = fieldName_f + " " + fieldType_f + ": " +  _array_ellypsis(value) ;

            } else {
                str = fieldName_f + " " + fieldType_f + ": " +(value ? value.toString() : "null".blue.bold);
            }
            data.lines.push(str);
        }
