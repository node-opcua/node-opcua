...
export const doDebug = !!process.env.DEBUG_CLASS;
...
export { check_schema_correctness };
...
export { check_options_correctness_against_schema };
...
function __field_category(field) {
  if (!field.category) {
    ...
    if (_enumerations[fieldType]) {
      ...
      console.log("-------------------------------------------------------------");
      Object.keys(require.cache).sort().forEach((k) => {
        console.log(k);
      });
            // xx console.log(f);
...
export { resolve_schema_field_types };
 ...
 * @return {*}
 */
export function initialize_field(field, value) {
 ...
 * @return {Array}
 */
export function initialize_field_array(field, valueArray) {
  const _t = field.schema;
