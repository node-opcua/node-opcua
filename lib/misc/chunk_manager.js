...
//  maxBodySize = plainBlockSize*floor((chunkSize–headerSize–signatureSize-1)/cipherBlockSize)-sequenceHeaderSize;
...
function clear(chunk) {
    for(var i=0;i<chunk.length;i++){ chunk.writeUInt8(0xFF,i);}
}

var ChunkManager_options = [
    "chunkSize","headerSize","signatureSize",
    "sequenceHeaderSize","cipherBlockSize","plainBlockSize",
    "compute_signature","encrypt_block","writeSequenceHeaderFunc","writeHeaderFunc"
];
/**
 * @class ChunkManager
...
function ChunkManager(options) {
    ...
    // { chunkSize : 32, headerSize : 10 ,signatureSize: 32 }
    ...
    this.signatureSize      = options.signatureSize   || 0;
    ...
    if (this.cipherBlockSize === 0 ) {
        // unencrypted block
        this.maxBodySize   = (this.chunkSize - this.headerSize - this.signatureSize  - this.sequenceHeaderSize);
        ...
        this.encrypt_block = options.encrypt_block;
        ...
        this.maxBodySize   = this.plainBlockSize * Math.floor(
            (this.chunkSize - this.headerSize - this.signatureSize - 1 ) / this.cipherBlockSize )- this.sequenceHeaderSize;

        // this is the formula proposed  by ERN
        this.maxBlock     =  Math.floor( ( this.chunkSize - this.headerSize ) / this.cipherBlockSize);
        this.maxBodySize  = this.plainBlockSize * this.maxBlock - this.sequenceHeaderSize - this.signatureSize - 1 ;
...
ChunkManager.prototype._write_signature = function (chunk) {
    ...
    if (this.compute_signature) {
        ...
        assert(signature.length === this.signatureSize);

        signature.copy(chunk,signature_start);

    } else {
        assert(this.signatureSize===0 , "expecting NO SIGN");
    }
};

ChunkManager.prototype._encrypt_buffer = function(buffer) {
...
/**
 * returns the number of extra bytes that will be added to the buffer
...
ChunkManager.prototype._extra_encryption_bytes = function() {
    ...
    if (this.plainBlockSize > 0) {
        var  length_to_encrypt = this.dataEnd - this.headerSize + this.signatureSize;
...
ChunkManager.prototype._encrypt = function(chunk) {
    ...
    if (this.plainBlockSize > 0) {
        ...
        var endEncryptionPos   = this.dataEnd + this.signatureSize;
        ...
        var encrypted_buf = this._encrypt_buffer(area_to_encrypt);
...
ChunkManager.prototype._append_padding = function () {
    ...
    if (this.plainBlockSize > 0) {
        ...
        // +---------------+---------------+-------------+-------+
        var curLength = this.sequenceHeaderSize + this.signatureSize + 1 +  this.cursor;
...
ChunkManager.prototype._postprocess_current_chunk = function () {
    ...
    var expected_length  = this.dataEnd + this.signatureSize + this._extra_encryption_bytes();
