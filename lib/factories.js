...
var _defaultType = [
    ...
    { name: "ExtensionObject",  encode: encodeExtensionObject, decode: decodeExtensionObject, defaultValue: function() { return null;} }
...
function encodeExtensionObject(object,stream){
    ...
    if (!object) {
        ec.encodeExpandedNodeId(object.makeExpandedNodeId(0),stream);
    } else {
        ec.encodeExpandedNodeId(object.encodingDefaultBinary,stream);
        object.encode(stream);
    }
}
function decodeExtensionObject(stream){
   var expandedId = ec.decodeExpandedNodeId(stream);
   if (expandedId.value ==0) {
        return null;
   }
   var object = exports.constructObject(expandedId);
