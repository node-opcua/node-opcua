...
var _exploreObject = function( kind_of_field,self,field,extra,data) {
    ...
    switch(kind_of_field) {
        ...
        case "enumeration":
            var typedEnum = _enumerations[fieldType].typedEnum;
            str = padding + r(fieldName,30) + " " + typedEnum.name + " " + value.key + " ( " + value.value + ")";
            data.lines.push(str);
            break;
        ...
        case "basic":
            if (value instanceof Buffer) {
                ...
                value = "<BUFFER>";
                data.lines.push(padding + r(fieldName,30) + str);
                data.lines.push(_hexDump);
            } else {
                str = padding + r(fieldName,30) + " " + r(fieldType,15) + " " + value;
                data.lines.push(str);
            }
            break;
        ...
        case "complex":
            if (field.subtype) {
               ...
               if (field.isArray){
                   data.lines.push(padding + r(fieldName,30) +  r(fieldType,15) +  ': [' );
                   var i=0;
