        ...
        if (obj.hasOwnProperty(field.name)) {
            ...
            if (_.isArray(member)) {
                ...
                for (var i = 0; i < nb; i++) {
                    ...
                    // var element = ___install_single_initial_value(field,{});
...
function installEnumProp(obj, name, Enum) {
    ...
    assert(!_.isFunction(obj[param]),'enum has alrady been installed cannot do it !!!');
...
function registerObject(schema) {
    ...
    var ClassConstructor = function (options) {
        assert(this!=undefined,"expecting using  'new' for a constructor");
...
var getConstructor = function (expandedId) {
    assert (expandedId && (expandedId.value in constructorMap), "cannot find constructor for expandedId " + util.inspect(expandedId,null,0) );
