...
function display_trace_from_this_projet_only() {
    ...
    stack = (stack.split("\n").filter(function(el){ return el.match(/node-opcua/)  && !el.match(/node_modules/);}));
    stack.shift();
...
function dump(obj) {
    console.log("\n",util.inspect(JSON.parse(JSON.stringify(obj)),{colors: true, depth:6}));
}
function dumpIf(condition, obj) {
    var util = require("util");
    if (condition) { dump(obj); }
}
