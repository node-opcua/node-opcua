...
_ResponseReceiver.prototype.handleResponse= function(data) {
    ...
    switch(msgType) {
        ...
        case "CLO":
        case "MSG":
            debugLog("Adding message to message builder");
            this._messageBuilder.feed(data);
            break;
        case "ERR":
            debugLog("Adding message to message builder");
            //xx this._messageBuilder.feed(data);
            var stream = new opcua.BinaryStream(data);
            var errCode = stream.readUInt32();
            var reason =  ec.decodeUAString(stream);
            self._terminate();
...
OPCUAClient.prototype.sendSecureOpcUARequest = function(msgType,msg,responseClass,callback) {
    ...
    secure_channel.chunkSecureMessage(msgType,requestId,msg,function(messageChunk){
