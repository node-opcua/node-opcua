...
var browseService = require("./browse_service");
/**
 * session.browse([{
...
OPCUASession.prototype.browse = function(nodesToBrowse,callback) {
    ...
    var request = new browseService.BrowseRequest({
    });

    nodesToBrowse.forEach(function(node) {
        var b = new browseService.BrowseDescription(node);
    ...
    var self  = this;
    self._client._secureChannel.performMessageTransaction(request, browseService.BrowseResponse,function(err,response){
        if(err) {
            callback(err,null);
        } else {
            // console.log(JSON.stringify(response.results,null," ").yellow.bold);
            callback(response.diagnosticInfos,response.results);
        }
    });
