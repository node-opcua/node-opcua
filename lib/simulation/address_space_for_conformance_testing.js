...
function _add_variable(server_engine, parent, varName, dataTypeName, current_value, isArray, extra_name) {
    ...
    var variable = server_engine.addVariable({
        componentOf:   parent,
        browseName:    name,
...
function add_mass_variables(server_engine, scalarFolder) {
    ...
    var scalarMass = server_engine.addFolder(scalarFolder, {
...
function add_simulation_variables(server_engine, scalarFolder) {
    ...
    var intervalVariable = server_engine.addVariable({
    ...
    var enabledVariable = server_engine.addVariable({
        componentOf: simulation,
        browseName: "Enabled",
...
function add_access_right_variables(server_engine, parentFolder) {

    var accessRight_Folder = server_engine.addFolder(parentFolder, {
...
function add_path_10deep(server_engine, simulation_folder) {

    var parent = simulation_folder;
    for (var i = 1; i < 10; i++) {
        var name = "Path_" + i.toString() + "Deep";

        var child = server_engine.addObjectInFolder(parent, {
            browseName: name,
...
function add_very_large_array_variables(server_engine, objectsFolder) {
    ...
    // add statics Array Variables
    var scalarStaticLargeArray = server_engine.addObjectInFolder(objectsFolder, {
...
//
function add_analog_data_items(server_engine, parentFolder) {
    ...
    // add statics Array Variables
    var analogItemFolder = server_engine.addObjectInFolder(parentFolder, {
...
build_address_space_for_conformance_testing = function (server_engine, options) {
    ...
    var simulationFolder = server_engine.addFolder(objectsFolder, "Simulation");

    add_access_right_variables(server_engine, simulationFolder);

    var scalarFolder = server_engine.addFolder(simulationFolder, {
        browseName: "Scalar",
    ...
    add_analog_data_items(server_engine, simulationFolder);

    add_path_10deep(server_engine,simulationFolder);
