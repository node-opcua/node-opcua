...
function validate_value_or_array(isArray,current_value,validatorFunc) {

    assert(_.isFunction(validatorFunc));
    if (isArray) {

        current_value.forEach(function (value, elementIndex) {
        ...
        // scalar
        if (!validatorFunc(current_value)) {
...
function _add_variable(server_engine, parent, dataTypeName, placeholder, isArray, extra_name) {
    ...
    validate_value_or_array(isArray,placeholder.current_value,validatorFunc);
...
function add_variable_array(server_engine, parent, dataTypeName, default_value) {
    ...
    var variable = _add_variable(server_engine, parent, dataTypeName, place_holder, true, "");
    assert(variable.accessLevel.key === "CurrentRead | CurrentWrite");
    assert(variable.userAccessLevel.key === "CurrentRead | CurrentWrite");
    assert(variable.historizing === false);
