...
function change_randomly() {
    ...
    value_to_change.forEach(function(element){
        ...
        element.current_value = ec["random"+dataType]();
...
function add_var(server_engine,parent,dataTypeName,defaultValue) {
    ...
    assert(validFunc(defaultValue) && "default value must be valid for dataType");

    var randomFunc = ec["random"+dataType];
    assert(_.isFunction(randomFunc) && "a random function must exist for basicType");
