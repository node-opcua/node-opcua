...
function defaultValidator(/*value*/) {return true;}
...
function getRandomFuncForType(dataType) {
    ...
    // console.log("xxxx dataType  ",dataType);
    switch (dataType) {
        case "Variant":
            return function()  { return new Variant(); };
        case "QualifiedName":
           return function() { return new QualifiedName({ name: ec.randomString() }); };
        case "LocalizedText":
            return function() { return new LocalizedText({text: ec.randomString() }); };
        case "XmlElement" :
            return function() {
...
function change_randomly() {
    value_to_change.forEach(function(element){ element.current_value = element.randomFunc();  });
}
...
build_address_space_for_conformance_testing = function (server_engine,options) {
    ...
    if (options.mass_variables) { add_mass_variables(server_engine,scalarFolder);   }
