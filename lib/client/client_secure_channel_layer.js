...
var MessageChunker = require("../misc/message_chunker").MessageChunker;
...
var secure_channel_service = require("../services/secure_channel_service");
...
var endpoints_service = require("../services/get_endpoints_service");
...
function _build_client_nonce() {
    ...
    // spec: This parameter shall have a length equal to key size used for the symmetric
    //       encryption algorithm that is identified by the securityPolicyUri
    var cryptoFactory = SecurityPolicy.getCryptoFactory(self.securityPolicy);
    if (!cryptoFactory) {
    ...
    var clientNonce = crypto.randomBytes(cryptoFactory.symmetricKeyLength) ;
...
ClientSecureChannelLayer.prototype._get_security_options_for_OPN = function() {
    ...
    var asymmetricSignatureLength = 128;
...
ClientSecureChannelLayer.prototype._get_security_options_for_MSG = function() {
    ...
    return SecurityPolicy.getOptionsForSymmetricSignAndEncrypt(self.securityMode,derivedClientKeys);
