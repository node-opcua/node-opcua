...
function _ResponseReceiver(request,expectedMsgTypes,responseClass,callback) {
    ...
    this._messageBuilder.on("message",function(response){
        ...
        if (response.responseHeader.requestHandle !== self._request.requestHeader.requestHandle) {
            ...
            var moreinfo = "Class = "+ response._description.name;
...
_ResponseReceiver.prototype._handle_response= function(message_chunk) {
    ...
    var _stream = new opcua.BinaryStream(message_chunk);
    ...
    debugLog("CLIENT RECEIVED " + (JSON.stringify(messageHeader)+"").yellow +  "\n" +
        hexDump(message_chunk).blue.bold);
...
/**
 * perform a OPC-UA message transaction.
 * During a transaction, the client send a request , the provided callback with be called
 * asynchronously with the reply from the server, or the error
 *
 * var secure_channel ;
 *
 * var message = new BrowseNameRequest({...});
 * secure_channel->performMessageTransaction("MSG",message,BrowseNameResponse,function(err,response) {
 *   if (err) {
...
ClientSecureChannelLayer.prototype._performMessageTransaction = function(msgType,requestMessage,responseClass,callback) {
    ...
    this._installReceiver(requestMessage,msgType,responseClass,callback);
    this._sendSecureOpcUARequest(msgType,requestMessage,function(){});
};
...
ClientSecureChannelLayer.prototype._sendSecureOpcUARequest = function(msgType,requestMessage,callback) {
    ...
    assert(options.tokenId>0 || msgType === "OPN");
    assert(this.tokenId   >0 || msgType === "OPN");
    ...
    self.messageChunker.chunkSecureMessage(msgType,options,requestMessage,function(messageChunk){
...
ClientSecureChannelLayer.prototype.close = function(callback)
{
    ...
    if (self._transport) {
        ...
        self._sendSecureOpcUARequest("MSG",request,function(){
