...
var ClientSecureChannelLayer = function (options) {
    ...
    self._messageBuilder.privateKey = crypto_utils.read_private_rsa_key("client_key.pem");
...
ClientSecureChannelLayer.prototype.create = function (endpoint_url, callback) {
    ...
    if (self.securityMode !== MessageSecurityMode.NONE) {
        ...
        assert(self.serverCertificate,"invalid server certificate");
...
ClientSecureChannelLayer.prototype._compute_derived_keys = function(){
...
ClientSecureChannelLayer.prototype._request_securityToken = function (is_initial, callback) {
    ...
    self._performMessageTransaction(msgType, msg, function (err, response) {
        if (!err) {
            ...
            self._messageBuilder.derivedKeys = self.derivedServerKeys;
 ...
 */
ClientSecureChannelLayer.prototype.getPrivateKey = function() { return this.parent.getPrivateKey(); };

ClientSecureChannelLayer.prototype.getCertificate = function() { return this.parent.getCertificate(); };
...
ClientSecureChannelLayer.prototype._construct_security_header = function() {
    ...
    switch(self.securityMode.value) {
        ...
        case MessageSecurityMode.SIGNANDENCRYPT.value:
            ...
            var thumbprint = crypto_utils.makeSHA1Thumbprint(self.serverCertificate);
            securityHeader = new AsymmetricAlgorithmSecurityHeader({
                securityPolicyUri: SecurityPolicy.toURI(self.securityPolicy),
                senderCertificate: self.getCertificate(), // certificate of the private key used to sign the message
...
ClientSecureChannelLayer.prototype._get_security_options_for_MSG = function() {
    ...
    assert(self.derivedClientKeys, "expecting valid derivedClientKeys");
    var derivedKeys = self.derivedClientKeys;
...
ClientSecureChannelLayer.prototype._sendSecureOpcUARequest = function (msgType, requestMessage, requestId) {
    ...
    if (msgType === "OPN") {
        ...
        var security_options = self._get_security_options_for_OPN();
        _.extend(options,security_options);

    } else {
        var security_options = self._get_security_options_for_MSG();
        _.extend(options,security_options);
