 ...
 * @uses MessageBuilder
 * @param options
 * @param [options.defaultSecureTokenLifetime=3000]
 * @constructor
 */
    ...
    transport.connect(endpoint_url, function (err) {
        if (err) {
            ...
            self._transport.on("message", function (message_chunk) {
                /**
                 * notify the owner that ClientSecureChannelLayer has received a message chunk
                 * @event receive_chunk
            ...
            //xx (undoing #9)
            //xx  self._transport.on("end", function (error) {
            //xx     self.emit("end", error);
            //xx });
 ...
 * @return {Number} generate the next request id
 */
