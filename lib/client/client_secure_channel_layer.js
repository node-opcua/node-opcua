...
ClientSecureChannelLayer.prototype._open_secure_channel_request = function (is_initial, callback) {
    ...
    self._performMessageTransaction(msgType, msg, function (error, response) {
        ...
        if (!error) {
            ...
            self.derivedKeys = SecurityPolicy.compute_derived_keys(self.serverNonce,self.clientNonce);
            self._messageBuilder.pushNewToken(self.securityToken,self.derivedKeys.derivedServerKeys);
