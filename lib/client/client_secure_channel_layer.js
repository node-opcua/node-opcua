...
function _ResponseReceiver(request,expectedMsgTypes,responseClass,callback) {
    ...
    this._messageBuilder.on("message",function(response){
        ...
        if (response.responseHeader.requestHandle !== self._request.requestHeader.requestHandle) {
            ...
            console.log(util.inspect(response,{colors:true,depth:10}));
        }
...
ClientSecureChannelLayer.prototype._request_securityToken = function(is_initial,callback) {
    ...
    var msgType =  (is_initial) ? "OPN" : "MSG";
    ...
    self._performMessageTransaction(msgType,msg, s.OpenSecureChannelResponse,function(err,response){
        if (!err) {
            ...
            //
            var liveTime =response.securityToken.revisedLifeTime;
            assert(liveTime);
            self._securityTokenTimeoutId = setTimeout(function() {
                console.log(" client: Security Token is about to expired, let's raise livetime_75 event ");
                self.emit("livetime_75");
