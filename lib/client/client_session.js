...
ClientSession.prototype.readAllAttributes = function (nodes, callback) {
  ...
  nodes.forEach((node) => {
    Object.keys(read_service.AttributeIds).forEach((key) => {
      const attributeId = read_service.AttributeIds[key];
      nodesToRead.push({
        nodeId: resolveNodeId(node),
        attributeId,
        indexRange: null,
...
ClientSession.prototype.call = function (methodsToCall, callback) {
  ...
  const request = new call_service.CallRequest({ methodsToCall });
...
const resultMask = browse_service.makeResultMask("ReferenceType");
