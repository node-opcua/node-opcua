...
ClientSession.prototype.write = function (nodesToWrite, callback) {
    ...
    var request = new write_service.WriteRequest({nodesToWrite});
...
ClientSession.prototype.readAllAttributes = function (nodes, callback) {
    ...
    nodes.forEach(node => {
        Object.keys(read_service.AttributeIds).forEach(key => {
...
ClientSession.prototype.call = function (methodsToCall, callback) {
    ...
    var request = new call_service.CallRequest({methodsToCall});
...
ClientSession.prototype.getArgumentDefinition = function (methodId, callback) {
    ...
    self.browse(browseDescription, (err, results) => {
        ...
        var inputArgumentRef = results[0].references.filter(r => r.browseName.name === "InputArguments");
        ...
        var outputArgumentRef = results[0].references.filter(r => r.browseName.name === "OutputArguments");
