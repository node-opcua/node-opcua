...
OPCUASession.prototype.read = function(nodesToRead,callback) {
    ...
    assert(_.isArray(nodesToRead) && nodesToRead.length>0);

    // coerce nodeIds
    nodesToRead.forEach(function(node){ node.nodeId = resolveNodeId(node.nodeId); });
...
OPCUASession.prototype.translateBrowsePath = function(browsePath,callback) {
    ...
    var constructBrowsePath = require("../address_space/address_space").constructBrowsePath;
