    ...
    function add_method(obj, reference, callback) {
        ...
        obj[name] = function functionCaller(inputArgs, callback) {
            ...
            var inputArguments = inputArgsDef.map(function (arg) {
                ...
                return { arrayType: arrayType, dataType: dataType, value: value};
            });
    ...
    async.series([
        ...
        function (callback) {
            ...
            if (clientObject.nodeClass === NodeClass.Variable) {
                proxyManager._monitor_value(clientObject);
            }
            callback();
        },
        function (callback) {
...
UAProxyManager.prototype._monitor_value = function(proxyObject) {


    if (proxyObject.nodeId.toString() !== "ns=0;i=2257") { return;}
    var self = this;
    ...
    var monitoredItem = self.subscription.monitor(itemToMonitor,monitoringParameters,requestedParameters)

    console.log("xxxxxx installing monitored Item");
    ...
    proxyObject.monitoredItem.on("changed",function(dataValue) {
        ...
        console.log(" Value Changed ".red,proxyObject.nodeId.toString() , proxyObject.browseName,proxyObject.dataValue.toString());
    });
