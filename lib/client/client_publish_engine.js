...
ClientSidePublishEngine.prototype._send_publish_request = function () {
    ...
    // In issue#40 (MonitoredItem on changed not fired), we have found that some server migh wrongly interpret
    ...
    var publish_request = new subscription_service.PublishRequest({
        requestHeader: { timeoutHint: 1000 }, // see note
        subscriptionAcknowledgements: subscriptionAcknowledgements
    });
    self.session.publish(publish_request, function (err, response) {
        if (err) {
            //xx console.log("xxxxxxxxxxxxxxxxxxx ClientSidePublishEngine.prototype._send_publish_request ".red ,err.message.yellow);
        } else {
...
ClientSidePublishEngine.prototype.registerSubscriptionCallback = function(subscriptionId,callback) {
    ...
    // todo  : send more than only publish request to server to cope with latency and create appropriated tests.
    //xx self.send_publish_request();
    //xx self.send_publish_request();
    //xx self.send_publish_request();
    //xx self.send_publish_request();
};
