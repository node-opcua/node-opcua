...
function _process_browse_response(task,callback) {
    ...
    self._defer_browse_node(nodeId, function (err, object) {
        ...
        if (!err) {
            ...
            for (i=0;i<object.references.length;i++) {
                ...
                _add_crawl_task.call(self,reference.referenceTypeId);
                _add_crawl_task.call(self,reference.nodeId);
            }
            emit_on_crawled.call(self,object);
        }
    });
    callback();
}

function _add_crawl_task(nodeId) {
...
NodeCrawler.prototype._inner_crawl = function(nodeId, user_data, end_callback) {
    ...
    _add_crawl_task.call(self,nodeId);
   //xx self.kick();
}
...
function remove_cycle(object, callback) {
                ...
                if (_.isArray(data[m])) {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        if (arr[i] && arr[i]._visited) {
                            arr[i] = "**** Already visited *** " + arr[i].nodeId.toString();
                            continue;
                        }
                        mark(arr[i]);
                    }
                    ...
                    if (data[m] && data[m]._visited) {
                        data[m] = "****Already visited *** *" + data[m].nodeId.toString();
                        continue;
                    }
                    mark(data[m]);
                }
    ...
    q.drain = function () {
        ...
        object_to_cleanup.forEach(function (d) {
            delete d._visited;
            delete d._id;
        });
        callback(null, object);
