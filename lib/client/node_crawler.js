...
NodeCrawler.prototype._defer_browse_node = function (cacheNode, referenceTypeId, actionOnBrowse) {
    ...
    self._objectToBrowse.push({
        ...
        //xx browseDirection: BrowseDirection.Forward,
...
NodeCrawler.prototype._process_single_browseResult = function (_objectToBrowse, browseResult) {
    ...
    cacheNode.references = browseResult.references;
    ...
    ///xx console.log("XXX cacheNode.nodeClass", cacheNode.toString());

    async.parallel([
          // function (callback) {
          //     if (cacheNode.hasOwnProperty("typeDefinition")) {
          //         return callback();
          //     }
          //     self._defer_browse_node(cacheNode.nodeId, hasTypeDefinitionId, function (err, browseName) {
...
NodeCrawler.follow = function (crawler, cacheNode, userData) {
    ...
    for (var i = 0; i < cacheNode.references.length; i++) {
        ...
        crawler.followReference(reference, userData);
    }
};
/***
 *
...
NodeCrawler.prototype._emit_on_crawled = function (cacheNode, userData) {
    var self = this;
    //xx console.log("xxxxx browsed = ",cacheNode.browseName.toString());
...
NodeCrawler.prototype._crawl_task = function (task, callback) {
    ...
    //xx var cacheNode =self._getCacheNode(nodeId);
    //xx if (cacheNode) {
    //xx     // object has already been browsed ...
    //xx     assert(cacheNode.browseName.name !== "pending");
    //xx     assert(cacheNode instanceof CacheNode);
    //xx     browse_node_action(null, cacheNode);
    //xx } else {
    //xx     cacheNode = self._createCacheNode(nodeId);
    self._defer_browse_node(cacheNode, referencesId/*hierarchicalReferencesId*/, browse_node_action);
    //xx }
    callback();
};
...
NodeCrawler.prototype.followReference = function (reference, userData) {
...
NodeCrawler.prototype.read = function (nodeId, callback) {
    ...
    function onBrowse(crawler, cacheNode, userData) {
        var i = 0;
        for (i = 0; i < cacheNode.references.length; i++) {
            var reference = cacheNode.references[i];
            crawler.followReference(reference, userData);
        }
    }
