...
TCP_transport.prototype._install_one_time_message_receiver = function (callback) {
    ...
    self._socket.once("data", function (data) {
        if (self._timerId) {
            ...
            the_callback = null;
        } // else : callback already processed after timing out
        ...
        //xx (undoing #9) self.emit("end", err);
    });
};
...
ServerTCP_transport.prototype._on_hello_message = function (helloMessage, callback) {
    ...
    if (helloMessage.protocolVersion < 0 || helloMessage.protocolVersion > self.protocolVersion) {
        ...
        self._abortWithError(StatusCodes.BadProtocolVersionUnsupported, " Server version is " + self.protocolVersion);
...
ServerTCP_transport.prototype.init = function (socket, callback) {
    ...
    self._install_one_time_message_receiver(function (err, data) {
        if (err) {
            ...
            self._abortWithError(StatusCodes.BadConnectionRejected, err.message);
            callback(err);
        } else {
            ...
            if (msgType === "HEL") {
                ...
                self._abortWithError(StatusCodes.BadCommunicationError, " Expecting 'HEL' message to initiate communication");
                callback(err);
            }
        }
    });
