...
TCP_transport.prototype._install_one_time_message_receiver = function (callback) {
    ...
    function _fullfill_pending_promises(err, data) {
    ...
    self._timerId = setTimeout(function () {
        _fullfill_pending_promises(new Error("Timeout in waiting for data on socket"));
    }, self.timeout);

    self._on_data_received = function (data) {
        _fullfill_pending_promises(null, data);
    };
    self._on_socket_ended = function (err) {
        _fullfill_pending_promises(new Error("Connection aborted - ended by server : " + (err ? err.message : "")));
    };
    self._on_socket_error = function (err) {
        _fullfill_pending_promises(new Error("Connection aborted - error on socket : " + (err ? err.message : "")));
    };
