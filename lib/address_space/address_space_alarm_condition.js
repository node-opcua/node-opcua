...
var conditions =require("./address_space_conditions");
var UAConditionBase = conditions.UAConditionBase;
var ConditionSnapshot = conditions.ConditionSnapshot;

var UAAcknowledgeableConditionBase = require("./address_space_acknowledgeable_conditions").UAAcknowledgeableConditionBase;
...
ConditionSnapshot.prototype.getSuppressedState = function() {
    var self = this;
    return self._set_twoStateVariable("suppressedState");
};
...
UAAlarmConditionBase.prototype.setMaxTimeShelved = function(duration) {
    ...
    self.maxTimeShelved.setValueFromSource({
        dataType: "Duration", // <= Duration is basic Type Double!
        value:duration
    });
};
...
function _timedShelve_method(inputArguments,context,callback) {
    ...
    assert(inputArguments[0].dataType == DataType.Duration);
    var proposedDuration= inputArguments[0].value;
...
exports.install = function (AddressSpace) {
    ...
    AddressSpace.prototype.instantiateAlarmCondition = function (alarmConditionTypeId, options,data ) {
        ...
        var self = this;
        var alarmNode = self.instantiateAcknowledgeableCondition(alarmConditionTypeId,options,data);
        ...
        // currently not in use. Recommended state names are described in Annex A.
        ...
        if (alarmNode.maxTimeShelved) {
            options.maxTimeShelved = options.maxTimeShelved || 60.0; // 60 seconds
            alarmNode.maxTimeShelved.setValueFromSource({dataType:"Duration",value:options.maxTimeShelved})
        }
        if(alarmNode.shelvingState) {
            var shelvingState = alarmNode.shelvingState;
