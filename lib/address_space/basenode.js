...
function _filter_by_nodeclass(self, references, nodeClassMask) {
    ...
    return references.filter(function (reference) {
        ...
        if (nodeClassName === "ReferenceType") {
            return false;
        }
        assert(typeof obj.nodeClass.key === "string");
...
BaseNode.prototype.browseNode = function(browseDescription) {
    ...
    if (do_debug) {  console.log("all references :"); dumpReferences(self,self.references); }
    ...
    referenceDescriptions = _constructReferenceDescription(address_space,references);
