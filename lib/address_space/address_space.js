...
AddressSpace.prototype._coerce_Type = function (dataType, typeMap, typeMapName, finderMethod) {
    ...
    if (!el) {
        ...
        var find = _.filter(typeMap, a => a === nodeId.value);
        /* istanbul ignore next */
        if (find.length !== 1) {
...
AddressSpace.prototype._identifyParentInReference = references => {
    ...
    var candidates = references.filter(ref => (ref.referenceType === "HasComponent" || ref.referenceType === "HasProperty")  &&
            ref.isForward === false);
...
AddressSpace.prototype.extractRootViews = function(node) {
    ...
    while(q.length) {
        ...
        var parentNodes = references.map(r => Reference._resolveReferenceNode(addressSpace,r));
...
AddressSpace.prototype.modelChangeTransaction = function(func) {
    ...
    function endModelChange(node) {
        ...
        if (this._modelChangeTransactionCounter === 0) {
            ...
            var nodes = _.uniq(this._modelChanges.map(c => c.affected));
