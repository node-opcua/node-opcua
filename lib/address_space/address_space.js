...
AddressSpace.prototype._coerce_Type = function(dataType,typeMap) {
    ...
    if (typeof dataType === "string") {
        ...
        if (!nodeId) {
            ...
            if (typeMap[dataType]) {
                ...
                console.log(" cannot find ",dataType ," in ", Object.keys(typeMap).join(" ") );
            }
        }
    } else {
        nodeId = dataType;
        var find = _.filter(typeMap,function(a)  {return a === dataType.value});
        if (find.length !== 1) {
            throw new Error(" cannot find " + dataType.toString() + " in " +  typeMap);
        }
...
AddressSpace.prototype._coerce_DataType = function(dataType) {
   return this._coerce_Type(dataType,DataTypeIds);
};
AddressSpace.prototype._coerce_VariableTypeIds = function(dataType) {
    return this._coerce_Type(dataType,VariableTypeIds);
};
...
AddressSpace.prototype.addVariable = function (parentObject, options) {
    ...
    assert(_.isArray(options.arrayDimensions));
