...
AddressSpace.prototype.registerNamespace = function(namespaceUri) {
    assert(this._namespaceArray.indexOf(namespaceUri) === -1);
...
AddressSpace.prototype.findReferenceType = function (refType) {
    ...
    if (refType instanceof NodeId) {
        ...
        assert(!object || (object.nodeClass === NodeClass.ReferenceType && object.browseName.toString() === refType));
    }
    return object;
};
...
function isNonEmptyQualifiedName(browseName) {
    if (!browseName) {return false; }
    if (typeof browseName === "string") { return browseName.length >=0; }
