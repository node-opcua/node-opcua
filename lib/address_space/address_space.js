...
AddressSpace.prototype.addVariable = function (parentObject, options) {
    ...
    typeDefinition = resolveNodeId(typeDefinition);
    ...
    if (typeof options.dataType === "string") {
        ...
        var n = self._aliases[options.dataType];
        if (n) {
            options.dataType = n;
        } else {
            var dataType = self._dataTypeMap[options.dataType];
            if (!dataType) {
                throw Error("addVariable  : dataType " + options.dataType + " doesn't exist in address space");
            }
            n = dataType.nodeId;
            options.dataType = n;
        }
