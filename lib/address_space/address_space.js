...
AddressSpace.prototype.extractRootViews = function(node) {
    ...
    while(q.length) {
        ...
        var browseResults = node.findReferencesEx("HierarchicalReferences",BrowseDirection.Inverse);
        var parentNodes = browseResults.map(function(r){ return addressSpace.findNode(r.nodeId); });

        parentNodes.forEach(function(parent){
            if (parent.nodeId === objectsFolder.nodeId) {
...
require("./address_space_add_two_state_variable").install(AddressSpace);
