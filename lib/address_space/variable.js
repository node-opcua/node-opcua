...
function _validate_dataType(actual_DataTypeNode, expected_dataTypeNode) {
    assert(actual_DataTypeNode && ( actual_DataTypeNode instanceof UADataType ), "expecting actual_DataTypeNode");
    assert(expected_dataTypeNode && ( expected_dataTypeNode instanceof UADataType ), "expecting expected_dataTypeNode");
...
Variable.prototype.readValue = function (indexRange, dataEncoding) {
    ...
    if (isGoodish(dataValue.statusCode)) {
        ...
        if (indexRange && dataValue.value.arrayType !== VariantArrayType.Scalar && _.isArray(variant.value)) {
...
Variable.prototype.writeValue = function (dataValue,indexRange, callback) {
    ...
    var actual_DataTypeNode = _dataType_toUADataType(this.__address_space, dataValue.value.dataType);
    ...
    if (indexRange && !indexRange.isEmpty()) {
        ...
        assert(_.isArray(self._dataValue.value.value));
    ...
    function bind_getter(options) {
        ...
        if (_.isFunction(options.get)) {                                   // variation 1
            ...
            self._internal_set_dataValue(new DataValue({
                value: ( options instanceof Variant ) ? options : new Variant(options),
                statusCode: StatusCodes.Good
            }));
        }
    }
