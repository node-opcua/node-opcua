...
function validateDataType(__address_space,dataTypeNodeId,variantDataType,nodeId) {
    ...
    if (destUADataType.isAbstract) {
        ...
        var builtInType = findBuiltInType(destUADataType.browseName).name;
        var builtInUADataType = __address_space.findDataType(builtInType);
    }
    ...
    self._timestamped_set_func = function(timestamped_value,callback) {
        ...
        self._set_func(timestamped_value.value,function(err,statusCode){
            if (!err) {
                ...
                self._internal_set_dataValue(timestamped_value);
            }
