...
function Variable(options) {
    ...
    assert(this.arrayDimensions === null || _.isArray( this.arrayDimensions));
    //xx new Variant({ dataType: DataType.UInt32, arrayType: VariantArrayType.Array , value : this.arrayDimensions });
...
function _validate_dataType(actual_DataTypeNode, expected_dataTypeNode) {
    assert(actual_DataTypeNode && ( actual_DataTypeNode instanceof UADataType ), " expectecting actual_DataTypeNode");
    assert(expected_dataTypeNode && ( expected_dataTypeNode instanceof UADataType ), " expectecting expected_dataTypeNode");
    return actual_DataTypeNode.isSubtypeOf(expected_dataTypeNode);
}
...
Variable.prototype.writeValue = function (variant, callback) {
    ...
    var actual_DataTypeNode = _dataType_toUADataType(this.__address_space, variant.dataType);
    var expected_DataTypeNode = this.__address_space.findObject(this.dataType);
    // verify_type_correctness
    // The value supplied for the attribute is not of the same type as the attribute's value.
    var isValidDataType = _validate_dataType(actual_DataTypeNode, expected_DataTypeNode);
    if (!isValidDataType) {
        console.log(" Expected dataType = ", expected_DataTypeNode.browseName);
        console.log(" Actual   dataType = ", actual_DataTypeNode.browseName);
    ...
    if (this._set_func) {
        ...
        assert(_.isFunction(this._set_func));
        if (this._set_func.length === 2) {
            // set func is async and is expecting a callback func like set(value,callback)
            if (callback) {
                assert(_.isFunction(callback));
                return this._set_func(variant, callback);
            } else {
                statusCode = this._set_func(variant) || StatusCodes.BadNotWritable;
            }
        } else {
            assert(this._set_func.length === 1);
            statusCode = this._set_func(variant) || StatusCodes.BadNotWritable;
        }
    } else {
...
function _not_writable_set_func(value) {
...
function _Variable_bind_with_timestamped_get(options) {
    ...
    self._set_func = options.set || _not_writable_set_func;
}
...
Variable.prototype.readValueAsync = function(callback) {
    ...
    self._readValueAsync_in_progress = (self._readValueAsync_in_progress )? true : false;
    if (self._readValueAsync_in_progress) {
        return;
    }
    self._readValueAsync_in_progress = true;
    ...
    func(function(err,timestamped_value){
        ...
        while (self.__waiting_callbacks.length) {
            self.__waiting_callbacks.shift()(err,timestamped_value);
        }
        //xx callback(err,timestamped_value);
        self._readValueAsync_in_progress = false;
    });
};
