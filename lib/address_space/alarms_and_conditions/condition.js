...
function _visit(self, node, prefix) {
    ...
    aggregates.forEach(function (aggregate) {
        ...
        if (aggregate instanceof UAVariable) {
            ...
            var key = (prefix + name).toLowerCase();
...
UAConditionBase.prototype.raiseNewCondition = function (options) {
    ...
    options.severity = options.hasOwnProperty("severity") ? options.severity : UAConditionBase.defaultSeverity;
    ...
    if (options.hasOwnProperty("retain")) {
        branch.setRetain(!!options.retain);
    }
 ...
 * @param options.conditionOf  {NodeId|UAObject}
