...
// Normally, the MethodId found in the Shelving child of the Condition instance and
// the NodeId of the Shelving object as the ObjectId are passed to the Call Service.
// However, some Servers do not expose Condition instances in the AddressSpace.
// Therefore all Servers shall also allow Clients to call the TimedShelve Method by specifying
// ConditionId as the ObjectId. The Method cannot be called with an ObjectId of
// the ShelvedStateMachineType Node.
//
// Signature:   TimedShelve([in] Duration ShelvingTime);
//
// ShelvingTime Specifies a fixed time for which the Alarm is to be shelved. The Server may
//    refuse the provided duration. If a MaxTimeShelved Property exist on the Alarm than the
//    Shelving time shall be less than or equal to the value of this Property.
// StatusCode :
//    Bad_ConditionAlreadyShelved The Alarm is already in TimedShelved state and
//              the system does not allow a reset of the shelved timer.
...
// and the NodeId of the Shelving object as the ObjectId are passed to the
// Call Service. However, some Servers do not expose Condition instances in the
// AddressSpace. Therefore all Servers shall also allow Clients to call the OneShotShelve
// Method by specifying ConditionId as the ObjectId. The Method cannot be called with an ObjectId
// of the ShelvedStateMachineType Node
...
UAAlarmConditionBase.instantiate = (addressSpace, alarmConditionTypeId, options, data) => {
  ...
  if (alarmNode.shelvingState) {
    ...
    /**
     * The SuppressedState and the ShelvingState together result in the SuppressedOrShelved
     * status of the Condition. When an Alarm is in one of the states, the SuppressedOrShelved
     * property will be set TRUE and this Alarm is then typically not displayed by the Client.
     * State transitions associated with the Alarm do occur, but they are not typically displayed
     * by the Clients as long as the Alarm remains in either the Suppressed or Shelved state.
     * @property suppressedState
  ...
  _update_suppressedOrShelved(alarmNode);
    ...
    /**
     * The optional Property MaxTimeShelved is used to set the maximum time that
     * an Alarm Condition may be shelved. The value is expressed as duration. Systems
     * can use this Property to prevent permanent Shelving of an Alarm. If this Property
     * is present it will be an upper limit on the duration passed into a TimedShelve Method call.
     * If a value that exceeds the value of this property is passed to the TimedShelve Method,
     * than a Bad_ShelvingTimeOutOfRange error code is returned on the call. If this Property
     * is present it will also be enforced for the OneShotShelved state, in that an Alarm Condition
     * will transition to the Unshelved state from the OneShotShelved state if the duration
     * specified in this Property expires following a OneShotShelve operation without a change of
     * any of the other items associated with the Condition.
     *
