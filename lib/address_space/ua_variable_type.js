...
function getModellingRule(node) {
    ...
    if (!r || r.length === 0) {
        console.log("Warning: HasModellingRule is missing in node ".yellow + node.browseName.toString().cyan);
        //xx console.log(node.toString());
        return "IGNORED"; // consider "IGNORED"
    }
    r = r[0];
...
function OnlyMandatoryChildrenOrRequestedOptionals(references,optionals,node) {
    ...
    var alreadyIn = references.filter(function(r){
        var n =addressSpace.findObject(r.nodeId);
...
function _initialize_properties_and_components(topMostType,self, references,optionals, extraInfo) {
    ...
    if (!baseTypeNodeId) {
        throw new Error("Object with nodeId " + self.nodeId + " - "  + self.browseName + " has no Type - (self.subtypeOf undefined)");
    }
...
function assertUnusedChildBrowseName(addressSpace,options) {
    ...
    function resolveOptionalObject(obj) {
        return  obj ? addressSpace._coerceObject(obj) : null;
    }
