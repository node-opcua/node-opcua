...
function OnlyMandatoryChildrenOrRequestedOptionals(instance,optionals,node) {
    ...
    var references = [].concat(_.map(instance._referenceIdx), _.map(instance._back_referenceIdx));
...
function _remove_unwanted_ref(references) {
    ...
    references = _.filter(references,function(reference) {
        ...
        //xx return !sameNodeId(reference.referenceTypeId,hasTypeDefintionNodeId) && !sameNodeId(reference.referenceTypeId,hasModellingRuleNodeId) ;
    });
    return references;
}
