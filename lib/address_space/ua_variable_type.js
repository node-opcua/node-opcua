...
function OnlyMandatoryChildren(references,node) {
    ...
    if (!r || r.length === 0) {
        ...
        return true; // consider "Mandatory"
    }
    r = r[0];
    return (r.browseName.toString() ==="Mandatory") ;
}


// install properties and components
// based on their ModelingRule
//  => Mandatory           => Installed
//  => Optional            => Not Installed
//  => OptionalPlaceHolder => Not Installed
function initialize_properties_and_components(topMostType,self, references) {
    ...
    initialize_properties_and_components(topMostType,baseType, references);

    // get properties and components from base class
    var r = self._clone_references(OnlyMandatoryChildren.bind(null,references));
...
UAVariableType.prototype.instantiate = function (options) {
    ...
    var references = [];
    initialize_properties_and_components(baseVariableType,self, references);
