...
var isUndefinedOrNull = require("lib/misc/utils").isUndefinedOrNull;
...
function UAVariableType(options) {
    ...
    this.isAbstract = isUndefinedOrNull(options.isAbstract) ? false : options.isAbstract;
...
function initialize_properties_and_components(topMostType,self, references,optionals) {
    ...
    var r = self._clone_references(OnlyMandatoryChildrenOrRequestedOptionals.bind(null,references,optionals));

    //xx console.log(self.browseName,"r = ", r.length);
    Array.prototype.push.apply(references, r);

    initialize_properties_and_components(topMostType,baseType, references,optionals);
