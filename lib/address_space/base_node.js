...
BaseNode.prototype.getComponentByName = function (browseName) {
    ...
    var properties = this.getComponents();
...
BaseNode.prototype.getFolderElementByName = function(browseName) {
    assert(typeof browseName === "string");
    var properties = this.getFolderElements();
    var select = properties.filter(function (c) {
        return c.browseName.toString() === browseName;
    });
    return select.length === 1 ? select[0] : null;
...
function _clone_collection(collection, references, referenceType,optionalFilter) {
 ...
 * @private
 */
BaseNode.prototype._clone_references = function (optionalFilter) {
    ...
    _clone_collection(components, references, "HasComponent" , optionalFilter);


    // now create instance members - HasProperty
    var properties = self.getProperties();
    _clone_collection(properties, references, "HasProperty", optionalFilter);
