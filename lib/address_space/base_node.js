...
function is_valid_reference(ref) {
    var hasRequestedProperties = ref.hasOwnProperty("referenceType") &&
        ...
        !util.isNullOrUndefined(ref.isForward);
...
Reference.prototype.toString = function (options) {
    ...
    return "REF{ " +node + " " + ( this.isForward ? "=>" : "<-" ) + refType + " }";
};
...
BaseNode.prototype.findReferencesEx = function (strReference, browseDirection)
{
    ...
    //xx browseDirection = browseDirection ? BrowseDirection.Forward :BrowseDirection.Inverse;
    assert(browseDirection === BrowseDirection.Forward || browseDirection === BrowseDirection.Inverse);
...
BaseNode.prototype.findReferences = function (strReference, isForward) {
    ...
    isForward = util.isNullOrUndefined(isForward) ? true : !!isForward;
...
BaseNode.prototype.addReference = function (reference) {
    ...
    reference = normalizeReferenceTypes(this.__address_space, [reference])[0];
...
BaseNode.prototype._toString = function(str,options)
{
    ...
    if (self.dataType) {
        ...
        var address_space = self.__address_space;
        var d = address_space.findObject(self.dataType);
    ...
    add(options.padding + "          references    : ".yellow + "  length =" + r.length);
    ...
    r.forEach(function (reference) {
        ...
        add(options.padding + "               +-> ".yellow + reference.toString(o) + " " + name.cyan);
