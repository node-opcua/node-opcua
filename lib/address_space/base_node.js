...
BaseNode.prototype.findReferencesAsObject = function (strReference, isForward) {
    ...
    function toObject(reference) {
        var obj = addressSpace.findObject(reference.nodeId);
...
function _propagate_ref(self, addressSpace, reference) {
    ...
    var related_node = addressSpace.findObject(reference.nodeId);
    if (related_node) {
...
function referenceTypeToString(addressSpace, referenceTypeId) {
    ...
    if (!referenceTypeId) {
        ...
        var referenceType = addressSpace.findObject(referenceTypeId);
...
function nodeIdInfo(addressSpace, nodeId) {
    ...
    var obj = addressSpace.findObject(nodeId);
...
function dumpReferences(addressSpace, references) {
    ...
    references.forEach(function (reference) {
        ...
        var ref = addressSpace.findObject(reference.referenceType);
        if (!ref) {
...
BaseNode.prototype.install_extra_properties = function () {
    ...
    function install_extra_properties_on_parent(ref) {
        var o = addressSpace.findObject(ref.nodeId);
...
function _clone_collection(collection, references, referenceType,optionalFilter, extraInfo) {
    ...
    collection.forEach(function (node) {
        ...
        var clone = node.clone(extraInfo);
 ...
 * @private
 */
BaseNode.prototype._clone_references = function (optionalFilter, extraInfo) {
    ...
    _clone_collection(components, references, "HasComponent" , optionalFilter, extraInfo);

    // now create instance members - HasProperty
    var properties = self.getProperties();
    _clone_collection(properties, references, "HasProperty", optionalFilter, extraInfo);

    assert(references.length <= components.length + properties.length);

    if (self.typeDefinition) {
        references.push({referenceType: "HasTypeDefinition", isForward: true, nodeId: self.typeDefinition});
    }
    return references;
};
...
BaseNode.prototype._clone = function (Constructor, options, extraInfo) {
    ...
    var references = self._clone_references(/*optionalFilter*/null,extraInfo);
...
BaseNode.prototype._toString = function(str,options)
{
    ...
    add(options.padding + "          description   : ".yellow + (self.description ? self.description.text.toString() : ""));


    if (self.dataType) {

        var addressSpace = self.__address_space;
        var d = addressSpace.findObject(self.dataType);
    ...
    add(options.padding + "         back_references: ".yellow + "  length =" + self._back_references.length + " ( references hold by other nodes pointing to me)");
    // backward reference
...
/*
    ...
    delete self._dataValue;
    delete self.browseName;
    delete self.__displayName;
    delete self.__description;
    delete self._references;
    delete self._back_references;
    delete self.__address_space;
    delete self.nodeId;
    delete self.valueRank;
    delete self.arrayDimension;
    delete self.eventNotifier;
    delete self.parentNodeId;
    delete self.historizing;
    delete self.accessLevel;
    delete self.userAccessLevel;
    delete self.minimumSamplingInterval;
    delete self.arrayDimensions;
*/
};
