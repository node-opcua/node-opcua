...
function _propagate_ref(self, addressSpace, reference) {
    ...
    // ------------------------------- FIlter out back reference when reference type is HasTypeDefinition
   // var referenceNode = Reference._resolveReferenceNode(addressSpace,reference);
    ...
    // becayse there are too many !
    if (
      //referenceNode &&
        (referenceType && referenceType.browseName.toString()=="HasTypeDefinition")
    ){ //&&(referenceNode.constructor.name === "UAVariableType" || referenceNode.constructor.name  === "UAObjectType")
