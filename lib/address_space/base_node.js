...
function same_reference(r1, r2) {
    ...
    return r1.referenceType === r2.referenceType &&
           same_nodeId(r1.nodeId,r2.nodeId);
}
...
function BaseNode(options) {
    ...
    this._references.forEach(function(ref) {
        self._referenceIdx[ref.hash] = ref;
    });
...
BaseNode.prototype._add_backward_reference = function (reference) {
    ...
    if (self._back_referenceIdx[h]) {
        ...
        console.log(" Warning !",self.browseName.toString());
        console.log("    ",reference.toString(opts));
        console.log(" already found in ===>");
        console.log(self._back_references.map(function(c){ return c.toString(opts);}).join("\n"));
        console.log("===>");
 ...
 * @return {NodeId[]}
 */
BaseNode.prototype.browseNodeByTargetName = function (relativePathElement) {
    ...
    function _check_reference(reference) {
        ...
        var referenceType =self.__address_space.findReferenceType(reference.referenceType);
        var referenceTypeId = referenceType.nodeId;

        if (same_nodeId(relativePathElement.referenceTypeId,referenceTypeId)) {
            return true;
        }
        if (relativePathElement.includeSubtypes) {
            var baseType = self.__address_space.findReferenceType(relativePathElement.referenceTypeId);
            if(referenceType.isSupertypeOf(baseType)) {
                return true;
            }
        }
        return false;
    ...
    if (self.subtypeOf) {
        ...
        var n = baseType.browseNodeByTargetName(relativePathElement);
