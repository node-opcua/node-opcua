...
// function isSameDataValue(dva,dvb) {
//
//     if (dva.statusCode != dvb.statusCode) {
//         return false;
//     }
//     if ((!dva.value && dvb.value) || (!dvb.value && dva.value)) {
//         return false;
//     }
//     return isSameVariant(va,vb);
// }
...
UAVariable.prototype._getEnumValues = function() {
    ...
    var dataTypeNode = self.__address_space.findDataType(self.dataType);

    var enumerationNode = self.__address_space.findDataType("Enumeration");
...
UAVariable.prototype.historyRead = function (historyReadDetails, indexRange, dataEncoding, continuationPoint, callback) {
    if (continuationPoint) {
        //TODO: use continutaion point manager to support continuation of historical reads
        callback(null, new HistoryReadResult({statusCode: StatusCodes.BadNotSupported}));
    } else if (typeof this["_historyRead"] != "function") {
        callback(null, new HistoryReadResult({statusCode: StatusCodes.BadNotSupported}));
    } else {
...
function _Variable_bind_with_simple_get(options) {
    ...
    function timestamped_get_func_from__Variable_bind_with_simple_get() {
        ...
        if (is_StatusCode(value)) {
            ...
            if (!self._dataValue || self._dataValue.statusCode !== StatusCodes.Good|| !isSameVariant(self._dataValue.value,value) ) {
