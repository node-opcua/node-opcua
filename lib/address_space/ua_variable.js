...
UAVariable.prototype.clone = function (extraInfo) {
...
function _bindComponent(variable,address_space,prop, name,dataTypeId, valueRank) {
    ...
    var dataType = address_space.findCorrespondingBasicDataType(dataTypeId);
...
UAVariable.prototype.bindExtensionObject = function() {
    ...
    if(dt.isSupertypeOf(structure)) {
        ...
        if (s.value.dataType === DataType.Null) {
            ...
            assert(s.value.value.constructor.name === address_space.constructExtensionObject(self.dataType).constructor.name);
        }
        ...
        dt.definition.forEach(function(field) {
            ...
            if (component.length === 1) {
                ...
                //xx var x = address_space.findObject(field.dataType).browseName.toString();
                //xx  var basicType = address_space.findCorrespondingBasicDataType(field.dataType);
                ...
                _bindComponent(self,address_space,component[0], name, field.dataType,field.valueRank );
            } else {
                assert(component.length===0);
                // create a variable
                // Note we may use ns=1;s=parentName/0:PropertyName

                var newProp = address_space.addVariable(self,{
                    browseName: field.name.toString(),
                    dataType: field.dataType
                });
                _bindComponent(self,address_space,newProp, name, field.dataType,field.valueRank );
