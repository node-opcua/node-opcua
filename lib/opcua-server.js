...
OPCUAServer.prototype._on_PublishRequest = function (request, channel)  {
    ...
    var server = this;
    if (!server.publishEngine) return;
    assert(server.publishEngine); // server.publishEngine doesn't exists, OPCUAServer has probably shut down already
    console.log(" recevied publish request");
    var engine = server.engine;
    assert(request._schema.name === "PublishRequest");
    server.publishEngine._on_PublishRequest(request,channel);
    server.publishEngine.once("publishResponse",function(request,response){
        channel.send_response("MSG", response);
    });
