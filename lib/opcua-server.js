...
OPCUAServer = function (options) {
    ...
    var endpoint = new OPCUAServerEndPoint(this, 26543 , {
...
// write services
OPCUAServer.prototype._on_Write = function(writeRequest,channel)  {
    var server = this;
    var engine = server.engine;
    assert(writeRequest._schema.name === "WriteRequest");
    assert(_.isArray(writeRequest.nodesToWrite));
    assert(writeRequest.nodesToWrite.length>0);
    assert(writeRequest.nodesToWrite[0]._schema.name === "WriteValue");

    var results = engine.write(writeRequest.nodesToWrite);

    assert(_.isArray(results));
    assert(results.length === writeRequest.nodesToWrite.length);
