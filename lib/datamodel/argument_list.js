...
function ArgumentList_verifyArguments(methodInputArguments,inputArguments) {
    ...
    if (methodInputArguments.length > inputArguments.length) {
        //xx console.log("xxxxxxxx ArgumentList_verifyArguments The client did  specify too many input arguments for the method.   expected : " +methodInputArguments.length + " actual = " + inputArguments.length );
        return { statusCode: StatusCodes.BadInvalidArgument};
    }

    if (methodInputArguments.length < inputArguments.length) {
        //xx console.log("xxxxxxxx ArgumentList_verifyArguments The client did not specify all of the input arguments for the method.   expected : " +methodInputArguments.length + " actual = " + inputArguments.length );
    ...
    for(var i = 0;i<methodInputArguments.length;i++) {
        ...
        //xx console.log("xxxxxxxx ArgumentList_verifyArguments checking argument " + i + "  expected : " + JSON.stringify(argDefinition)+ " actual = " + JSON.stringify(arg));
        if (!isArgumentValid(argDefinition,arg)) {
            //xx console.log("xxxxxxxx ArgumentList_verifyArguments The client did specify a argument with the wrong data type.   expected : " +argDefinition.dataType+ " actual = " + arg.dataType);
