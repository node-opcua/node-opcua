...
function construct_numeric_range_from_string(str) {
    ...
    if (values.length === 1) {
        ...
        if (elements[0].type === NumericRangeType.ArrayRange && elements[1].type === NumericRangeType.ArrayRange) {

            return {type: NumericRangeType.MatrixRange, value: [elements[0].value, elements[1].value]};

        } else if (elements[0].type === NumericRangeType.SingleValue && elements[1].type === NumericRangeType.SingleValue) {
...
NumericRange.prototype.toEncodeableString = function()
{
    switch (this.type) {
        ...
        case NumericRangeType.InvalidRange:
            return this.value || "-1:-3"; // simulate an invalid Item
        default:
            return null;
    }
};
