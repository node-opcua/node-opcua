...
NumericRange.prototype.extract_values = function (array) {
    ...
    case NumericRangeType.SingleValue:
        ...
        if (index >= array.length) {
            //xx console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX A".red,this.toString(),array.length);
            return {
                array: [],
    ...
    case NumericRangeType.ArrayRange:
        ...
        var high_index = this.value[1]+1;
        assert(low_index < high_index);
        if ((high_index) >= array.length) {
            //xx console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX B".red,this.toString(),array.length);
            return {
                array: [],
                statusCode: StatusCodes.BadIndexRangeNoData
            };
...
NumericRange.prototype.set_values = function (arrayToAlter,newValues) {
    ...
    assert(_.isArray(arrayToAlter));
    assert(_.isArray(newValues));

    function insertInPlace(arrayToAlter,low,high,newValues) {
    ...
    if (high_index >= arrayToAlter.length) {
        return { array: [], statusCode: StatusCodes.BadIndexRangeInvalid };
    }
    return {
