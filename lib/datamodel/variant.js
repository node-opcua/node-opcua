...
function assert_valid_UInt32(value) {
    assert(_.isFinite(value));
    assert(value >=0 && value < 0xFFFFFFFF );
}

function assert_valid_Int32(value) {
    assert(_.isFinite(value));
    assert(value >=-0x7FFFFFFF && value < 0x7FFFFFFF );
}

function assert_valid_UInt16(value) {
    assert(_.isFinite(value));
    assert(value >= -0x0 && value < 0xFFFF);
}

function assert_valid_Int16(value) {
    assert(_.isFinite(value));
    assert(value >=-0x7FFF && value < 0x7FFF );
}

function assert_valid_UInt8(value) {
    assert(_.isFinite(value));
    assert(value >= -0x0 && value < 0xFF);
}

function assert_valid_Int8(value) {
    assert(_.isFinite(value));
    assert(value >=-0x7F && value < 0x7F );
}

function assert_valid_variant(dataType,value) {

    switch(dataType)  {
        case DataType.UInt32:
            assert_valid_UInt32(value);
            break;
        case DataType.Int32:
            assert_valid_Int32(value);
            break;
        case DataType.UInt16:
            assert_valid_UInt16(value);
            break;
        case DataType.Int16:
            assert_valid_Int16(value);
            break;
        case DataType.Byte:
            assert_valid_UInt8(value);
            break;
        case DataType.Int8:
            if ( value === undefined) {
            ...
            assert_valid_Int8(value);
            break;
    }
}
exports.assert_valid_variant = assert_valid_variant;
...
var Variant_Schema = {
    ...
    construct_hook: function( options) {
        ...
        if ( options.arrayType && options.arrayType !== VariantArrayType.Scalar) {
            ...
            assert_valid_variant(options.dataType,options.value);
        }
        return options;
    },
    isValid: function(self) {
        assert_valid_variant(self.dataType,self.value);
        return true;
    }
