...
//xx        console.log(util.inspect(obj,{colors:true,depth:10}));
    }
...
function Variable(options) {
    ...
    this.valueRank = options.valueRank;  // UInt32
    this.arrayDimensions = [];
    //xx console.log(options.dataType,this.dataType.toString());
...
Variable.prototype.readAttribute = function (attributeId) {
    ...
    switch (attributeId) {
        ...
        case AttributeIds.Historizing:
            options.value = { dataType: DataType.Boolean, value: false };
            options.statusCode = StatusCodes.Good;
            break;
        default:
            return BaseNode.prototype.readAttribute.call(this,attributeId);
...
AddressSpace.prototype._register = function (object) {
    ...
    if(this._browsename_index.hasOwnProperty(object.browseName)) {
        console.log("registering ",object.browseName,object.nodeId.toString());
        throw new Error("browseName already registered");
    }
    assert(!this._browsename_index.hasOwnProperty(object.browseName),"browseName already registered");
    this._nodeid_index[object.nodeId.toString()] = object;
    this._browsename_index[object.browseName] = object;
};
...
AddressSpace.prototype._createObject = function(options) {
    ...
    assert(this.findObjectByBrowseName(options.browseName) === obj);
