...
Variable.prototype.readAttribute = function (attributeId) {
    ...
    switch (attributeId) {
        ...
        case AttributeIds.DataType:
            ...
            options.value = { dataType: DataType.Int32, value: variant.dataType.value};
            options.statusCode = StatusCodes.Good;
            break;
        case AttributeIds.ValueRank:
            var variant = this.get_variant();
            options.value = { dataType: DataType.Int32, value: variant.arrayType.value };
            options.statusCode = StatusCodes.Good;
            break;
        case AttributeIds.ArrayDimensions:
            options.value = { dataType: DataType.UInt32, value: 0 };
...
AddressSpace.prototype.findByBrowseName = function(browseNameToFind) {
    var el= _.find(this._nodeid_index,function(el){
        return el.browseName === browseNameToFind;
    });
    return el;
};
