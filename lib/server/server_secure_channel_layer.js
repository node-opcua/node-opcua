...
ServerSecureChannelLayer.prototype.send_response = function (msgType, response, message, callback) {
    ...
    if(self.msgType === "OPN") {
        ...
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            options.plainBlockSize =  cryptoFactory.plainBlockSize;
            options.cipherBlockSize = cryptoFactory.cipherBlockSize;
        ...
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            options.signatureLength = derivedServerKeys.signatureLength;
            options.signingFunc= function(chunk) {
                return crypto_utils.makeMessageChunkSignatureWithDerivedKeys(chunk,derivedServerKeys);
            };
        }
        if (self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            options.plainBlockSize = derivedServerKeys.encryptingBlockSize;
            options.cipherBlockSize= derivedServerKeys.encryptingBlockSize;
            options.encrypt_buffer = function (chunk) {
                return crypto_utils.encryptBufferWithDerivedKeys(chunk,derivedServerKeys);
            };
        }
    }
...
function _handle_OpenSecureChannelRequest(message) {
    ...
    this.derivedKeys = SecurityPolicy.compute_derived_keys(this.serverNonce,this.clientNonce);
    this.messageBuilder.pushNewToken(this.securityToken,this.derivedKeys.derivedClientKeys);
    ...
    self.messageChunker = new MessageChunker({
        ...
        derivedKeys: self.derivedKeys.derivedServerKeys
    });
