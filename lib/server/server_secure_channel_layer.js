 ...
 * @param [options.timeout = 2000] {Number} timeout
...
function ServerSecureChannelLayer(options) {
    ...
    options.privateKey = options.privateKey || crypto_utils.read_private_rsa_key("key.pem");
    self.messageBuilder.privateKey = options.privateKey;
    ...
    self.messageBuilder.on("error", function(err) {
        console.log("error ".red,err.message.yellow,err.stack);
 ...
 */
ServerSecureChannelLayer.prototype.getCertificate = function() { return this.parent.getCertificate(); };
 ...
 */
ServerSecureChannelLayer.prototype.getPrivateKey = function() { return this.parent.getPrivateKey(); };
...
ServerSecureChannelLayer.prototype._add_new_security_token = function () {
    ...
    self._stop_security_token_watch_dog();
    self.lastTokenId += 1;

    var securityToken = new s.ChannelSecurityToken({
    ...
    self._start_security_token_watch_dog();
};
...
ServerSecureChannelLayer.prototype._prepare_security_token = function (openSecureChannelRequest) {
    ...
    if (openSecureChannelRequest.requestType === s.SecurityTokenRequestType.RENEW) {

        // Release 1.02  page 41  OPC Unified Architecture, Part 6:
        // The  Server  shall  reject renew requests if the  SenderCertificate  is not
        // the same as the   one used to create the  SecureChannel

        console.log("RENEW ----->");
        // creates a new SecurityToken for an existing ClientSecureChannelLayer .
        self._stop_security_token_watch_dog();

    } else if (openSecureChannelRequest.requestType === s.SecurityTokenRequestType.ISSUE) {
...
ServerSecureChannelLayer.prototype._set_lifetime = function(requestedLifetime) {
...
ServerSecureChannelLayer.prototype._stop_open_channel_watch_dog = function() {
...
ServerSecureChannelLayer.prototype._stop_security_token_watch_dog = function() {
...
ServerSecureChannelLayer.prototype._start_security_token_watch_dog = function() {
    ...
    self._securityTokenTimeout = setTimeout(function() {
        ...
        console.log(" Server will now refuse message with token ",self.securityToken);
        self._securityTokenTimeout = null;
...
ServerSecureChannelLayer.prototype._cleanup_pending_timers = function() {
    ...
    self._stop_open_channel_watch_dog();
};
...
ServerSecureChannelLayer.prototype._send_chunk = function(callback,messageChunk) {
...
ServerSecureChannelLayer.prototype.send_response = function (msgType, response, message, callback) {
    ...
    if(self.msgType === "OPN") {
        ...
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            options.signingFunc= function(chunk) { return self.sign(chunk); };
            assert(self.receiverPublicKey);
            // create a public key from the client certificate
            options.plainBlockSize= 128-11;
            options.cipherBlockSize= 128;
            options.encrypt_buffer = function (chunk) {
                return crypto_utils.publicEncrypt_long(chunk, self.receiverPublicKey,options.cipherBlockSize,11);
            }
        }
    } else if (self.msgType === "MSG" ) {

        // symmetric
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {

            assert(self.derivedServerKeys);
            options.signatureLength = self.derivedServerKeys.signatureLength;
            options.signingFunc= function(chunk) {
                return crypto_utils.makeMessageChunkSignatureWithDerivedKeys(chunk,self.derivedServerKeys);
            };

        }
        if (self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            assert(self.derivedServerKeys);
            options.plainBlockSize = self.derivedServerKeys.encryptingBlockSize;
            options.cipherBlockSize= self.derivedServerKeys.encryptingBlockSize;
            options.encrypt_buffer = function (chunk) {
                return crypto_utils.encryptBufferWithDerivedKeys(chunk,self.derivedServerKeys);
            };
        }
    }
    ...
    self.messageChunker.chunkSecureMessage(msgType, options, response, self._send_chunk.bind(self,callback));
};
...
ServerSecureChannelLayer.prototype._prepare_security_header = function(request,message) {
...
ServerSecureChannelLayer.prototype._compute_derived_keys = function(){

    // calculate derived keys
    this.derivedClientKeys = null;
    this.derivedServerKeys = null;
    if (this.clientNonce && this.serverNonce) {
        ...
        this.derivedClientKeys = crypto_utils.computeDerivedKeys(this.serverNonce,this.clientNonce,options);
        this.derivedServerKeys = crypto_utils.computeDerivedKeys(this.clientNonce,this.serverNonce,options);

        // expose derivedKey to use for symmetric sign&encrypt
        // to help us decrypting and verifying messages received from client
        this.messageBuilder.pushNewToken(this.securityToken,this.derivedClientKeys);
    }
};

ServerSecureChannelLayer.prototype._handle_OpenSecureChannelRequest = function (message) {
    ...
    self._set_lifetime(request.requestedLifetime);

    self._prepare_security_token(request);

    self._compute_derived_keys();

    self._prepare_security_header(request,message);
    ...
    self.messageChunker = new MessageChunker({
        ...
        derivedKeys: self.derivedServerKeys
    });
...
var _on_common_message = function (request, msgType) {
    ...
    if (msgType === "CLO" && request._schema.name === "CloseSecureChannelRequest") {
        ...
        self._handle_OpenSecureChannelRequest(message);
    } else {
...
ServerSecureChannelLayer.prototype._check_receiverCertificateThumbprint =  function (securityHeader) {
...
ServerSecureChannelLayer.prototype._check_sender_certificate =  function (securityHeader) {
...
ServerSecureChannelLayer.prototype._on_initial_OpenSecureChannelRequest = function (message, callback) {
    ...
    if (self.securityMode.value !== MessageSecurityMode.NONE.value) {
        ...
        if (!self._check_receiverCertificateThumbprint(self.securityHeader)) {
            callback(new Error("Invalid receiver certificate  thumbprint : the thumbprint doesn't match server certificate !"));
            return;
        }
        if (!self._check_sender_certificate(self.securityHeader)) {
