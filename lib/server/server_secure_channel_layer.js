...
function check_certificate_for_sign_and_encrypt(securityHeader) {
    if (securityHeader.receiverCertificateThumbprint === null ) {
        return false; // missing Thumbprint
    }
    if (securityHeader.senderCertificate === null ) {
        return false; // missing certificate
    }
    // todo verify thumbprint
...
ServerSecureChannelLayer.prototype._on_initial_OpenSecureChannelRequest = function (message, callback) {
    ...
    switch(self.securityMode.value) {
        ...
        case MessageSecurityMode.SIGNANDENCRYPT.value:
            if (!check_certificate_for_sign_and_encrypt(self.securityHeader)) {
