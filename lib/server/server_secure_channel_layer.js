...
ServerSecureChannelLayer.prototype.send_response = function (msgType, response, message, callback) {
    ...
    if(self.msgType == "OPN") {
        ...
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            var params = {signatureLength: signatureLength,algorithm:  "RSA-SHA1",  privateKey:self.getPrivateKey() };

            options.signatureLength = signatureLength;
        ...
        if (self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {
            ...
            options.encrypt_block = function (chunk) {
                return crypto_utils.publicEncrypt(chunk, self.receiverPublicKey);
            }
        }
    } else if (self.msgType === "MSG" && self.derivedKeys ) {

        // symmetric
        if (self.securityMode === MessageSecurityMode.SIGN || self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {

            options.signatureLength = self.derivedKeys.signatureLength;
            options.signingFunc= function(chunk) {
                return crypto_utils.makeMessageChunkSignatureWithDerivedKeys(chunk,self.derivedKeys);
            };

        }
        if (self.securityMode === MessageSecurityMode.SIGNANDENCRYPT) {

            options.plainBlockSize = self.derivedKeys.encryptingBlockSize;
            options.cipherBlockSize= self.derivedKeys.encryptingBlockSize;
            options.encrypt_block = function (chunk) {
                return crypto_utils.encryptBufferWithDerivedKeys(chunk,self.derivedKeys);
            }
