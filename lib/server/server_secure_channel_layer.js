 ...
 * @param request
 * @param message
 * @returns {AsymmetricAlgorithmSecurityHeader}
 * @private
 */
...
function _handle_OpenSecureChannelRequest(message,callback) {
    ...
    if (self.securityMode.value !== MessageSecurityMode.NONE.value) {
        ...
        var certificate_status = _check_certificate_validity(self.clientSecurityHeader.senderCertificate);
        if (StatusCodes.Good !== certificate_status) {
            var description = "Sender Certificate Error";
    ...
    self.send_response("OPN", response, message,function(err) {
        ...
        if (response.responseHeader.serviceResult !== StatusCodes.Good) {
            //xx console.log("xxxx Response Send ...".yellow.bold,response.toString(),err);
            self.close();
        }
        callback();
