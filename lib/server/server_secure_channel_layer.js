...
function _handle_OpenSecureChannelRequest(message) {
    ...
    if (cryptoFactory) {
        ...
        self.serverNonce = crypto.randomBytes(Math.max(cryptoFactory.symmetricKeyLength,32));
        //xx assert(self.clientNonce.length === self.serverNonce.length);
...
function _on_initial_OpenSecureChannelRequest(message, callback) {
    ...
    if (!has_endpoint) {
        ...
        return _send_error.call(this,StatusCodes.BadSecurityPolicyRejected, description,message, callback);
    }
    ...
    if (self.securityMode.value !== MessageSecurityMode.NONE.value) {
        ...
        if (!_check_receiverCertificateThumbprint.call(self,self.securityHeader)) {
            ...
            return _send_error.call(this,StatusCodes.BadCertificateInvalid,description, message, callback);
        }
        ...
        if (StatusCodes.Good !== certificate_status) {
            ...
            return _send_error.call(this,certificate_status,description, message, callback);
        }
    }
