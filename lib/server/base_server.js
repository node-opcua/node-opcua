...
OPCUABaseServer.prototype.on_request = function(message,channel) {
    ...
    // wrap channel.send_response so we can intercept its call and  emit the "response" event.
    var old_method =channel.send_response;
    channel.send_response = function(msg,response,message) {
        old_method.call(this,msg,response,message);
        self.emit("response",response);
        channel.send_response= old_method;
    };
...
OPCUABaseServer.prototype.getDiscoveryUrls = function(channel) {
    assert(channel);
    var discoveryUrls = channel.parent.endpointDescriptions().map(function(e) { return e.endpointUrl; });
