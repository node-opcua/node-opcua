...
function findUserTokenByPolicy(endpoint_description, policyId) {
    ...
    var r = _.filter(endpoint_description.userIdentityTokens, userIdentity => {
...
OPCUAServer.prototype._on_ActivateSessionRequest = function (message, channel) {
    ...
    server.isUserAuthorized(channel, session, request.userIdentityToken, (err, authorized) => {
        ...
        if (!authorized) {
            ...
            var userIdentityTokenPasswordRemoved=userIdentityToken => {
...
OPCUAServer.prototype._apply_on_SubscriptionIds = function (ResponseClass,message,channel,action_to_perform) {
    ...
    this._apply_on_SessionObject(ResponseClass, message, channel, session => {
        ...
        var results = subscriptionIds.map(subscriptionId => action_to_perform(session,subscriptionId));
...
OPCUAServer.prototype._on_DeleteMonitoredItemsRequest = function (message, channel) {
    ...
    this._apply_on_Subscription(DeleteMonitoredItemsResponse, message, channel, (session, subscription) => {
        ...
        var results = request.monitoredItemIds.map(monitoredItemId => subscription.removeMonitoredItem(monitoredItemId));
...
OPCUAServer.prototype._on_SetMonitoringModeRequest = function (message, channel) {
    ...
    this._apply_on_Subscription(RepublishResponse, message, channel, (session, subscription) => {
        ...
        var results = request.monitoredItemIds.map(monitoredItemId => {
...
OPCUAServer.prototype._on_TranslateBrowsePathsToNodeIdsRequest = function (message, channel) {
    ...
    this._apply_on_SessionObject(TranslateBrowsePathsToNodeIdsResponse, message, channel, session => {
        ...
        var browsePathResults = request.browsePath.map(browsePath => server.engine.browsePath(browsePath));
...
OPCUAServer.prototype._registerServer = function (discovery_server_endpointUrl, isOnLine, outer_callback) {
    ...
    function findSecureEndpoint(endpoints) {
        var endpoint = endpoints.filter(e => e.securityMode === MessageSecurityMode.SIGNANDENCRYPT);
