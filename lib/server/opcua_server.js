...
Factory.prototype.constructObject = function(id) {
    var obj = factories.constructObject(id);
    return obj;
};
 ...
 * @param [options.timeout=10000] {Number} the HEL/ACK transaction timeout in ms. Use a large value ( i.e 15000 ms) for
...
function OPCUAServer(options) {
    ...
    var endPoint = new OPCUAServerEndPoint(port, {
...
function monitoredItem_read_and_record_value(self,oldValue,node, itemToMonitor) {
    ...
    if (dataValue.statusCode === StatusCodes.Good) {
        if (!_.isEqual(dataValue.value, oldValue)) {
            self.recordValue(dataValue.value,dataValue.sourceTimestamp,dataValue.sourcePicoseconds);
        }
    } else {
...
function monitoredItem_read_and_record_value_async(self, oldValue, node, itemToMonitor) {
    ...
    node.readValueAsync(function(err,timestamped_value){
...
function build_scanning_node_function(engine, itemToMonitor) {
    ...
    return  function (oldValue) {
        ...
        return monitoredItem_read_and_record_value_func(this, oldValue, node, itemToMonitor);
    }
...
var ArgumentList_verifyArguments = require("lib/datamodel/argument_list").ArgumentList_verifyArguments;
...
function callMethod(session,callMethodRequest,callback) {
    ...
    var response = ArgumentList_getMethodDeclaration(address_space,objectId,methodId);
    ...
    response  = ArgumentList_verifyArguments(methodInputArguments,inputArguments);
    if (response.statusCode != StatusCodes.Good) {
        return callback(null,{ statusCode: response.statusCode} );
    }
...
OPCUAServer.prototype._on_CallRequest = function (message, channel) {
    ...
    this._apply_on_SessionObject(RepublishResponse,message, channel, function (session) {
        ...
        if (request.methodsToCall.length === 0) {
            // BadNothingToDo
            var response = new CallResponse({ responseHeader: {serviceResult: StatusCodes.BadNothingToDo} });
        ...
        async.map(request.methodsToCall,callMethod.bind(server,session),function(err,results){
            assert(_.isArray(results));
            var response = new CallResponse({results: results});
            channel.send_response("MSG", response, message);
