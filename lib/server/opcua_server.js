...
const RegisterNodesRequest = browse_service.RegisterNodesRequest;
const RegisterNodesResponse = browse_service.RegisterNodesResponse;
const UnregisterNodesRequest = browse_service.UnregisterNodesRequest;
const UnregisterNodesResponse = browse_service.UnregisterNodesResponse;
...
const MonitoredItem = require("lib/server/monitored_item").MonitoredItem;

const View = require("lib/address_space/view").View;

const crypto = require("crypto");

const dump = require("lib/misc/utils").dump;
...
Factory.prototype.constructObject = id => factories.constructObject(id);
...
const computeSignature = require("lib/misc/security_policy").computeSignature;
const verifySignature = require("lib/misc/security_policy").verifySignature;
...
const split_der = require("lib/misc/crypto_explore_certificate").split_der;
...
OPCUAServer.prototype._on_CreateSessionRequest = function (message, channel) {
  ...
  function validate_applicationUri(applicationUri, clientCertificate) {
    ...
    const exploreCertificate = require("lib/misc/crypto_explore_certificate").exploreCertificate;
...
const securityPolicy_m = require("lib/misc/security_policy");
...
function adjustSecurityPolicy(channel, userTokenPolicy_securityPolicyUri) {
  ...
  let securityPolicy = securityPolicy_m.fromURI(userTokenPolicy_securityPolicyUri);
...
OPCUAServer.prototype.isValidUserNameIdentityToken = function (channel, session, userTokenPolicy, userIdentityToken) {
  ...
  const cryptoFactory = securityPolicy_m.getCryptoFactory(securityPolicy);
  if (!cryptoFactory) {
...
OPCUAServer.prototype.makeServerNonce = () => crypto.randomBytes(32);
 ...
 * @private
 */
OPCUAServer.prototype._apply_on_SessionObject = (ResponseClass, message, channel, action_to_perform) => {
...
const getMethodDeclaration_ArgumentList = require("lib/datamodel/argument_list").getMethodDeclaration_ArgumentList;
const verifyArguments_ArgumentList = require("lib/datamodel/argument_list").verifyArguments_ArgumentList;
...
OPCUAServer.prototype._on_Cancel =  (message, channel) => g_sendError(channel, message, session_service.CancelResponse, StatusCodes.BadNotImplemented);
...
OPCUAServer.prototype._on_AddNodes = (message, channel) => g_sendError(channel, message, node_managment_service.AddNodesResponse, StatusCodes.BadNotImplemented);

OPCUAServer.prototype._on_AddReferences = (message, channel) => g_sendError(channel, message, node_managment_service.AddReferencesResponse, StatusCodes.BadNotImplemented);
OPCUAServer.prototype._on_DeleteNodes = (message, channel) => g_sendError(channel, message, node_managment_service.DeleteNodesResponse, StatusCodes.BadNotImplemented);
OPCUAServer.prototype._on_DeleteReferences = (message, channel) => g_sendError(channel, message, node_managment_service.DeleteReferencesResponse, StatusCodes.BadNotImplemented);


// Query Service
OPCUAServer.prototype._on_QueryFirst = (message, channel) => g_sendError(channel, message, query_service.QueryFirstResponse, StatusCodes.BadNotImplemented);
OPCUAServer.prototype._on_QueryNext = (message, channel) => g_sendError(channel, message, query_service.QueryNextResponse, StatusCodes.BadNotImplemented);


OPCUAServer.prototype._on_HistoryUpdate = (message, channel) => g_sendError(channel, message, historizing_service.HistorizingUpdateResponse, StatusCodes.BadNotImplemented);
...
OPCUAServer.prototype._registerServer = function (discovery_server_endpointUrl, isOnLine, outer_callback) {
  ...
  const async = require("async");
  let MessageSecurityMode = endpoints_service.MessageSecurityMode;
  const securityPolicy_m = require("lib/misc/security_policy");
  const SecurityPolicy = securityPolicy_m.SecurityPolicy;
