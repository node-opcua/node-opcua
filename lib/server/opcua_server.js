 ...
 *
 * @param [options.nodeset_filename]{Array<String>|<Sring>} the nodeset.xml files to load
...
function OPCUAServer(options) {
    ...
    // build Info
    buildInfo = _.clone(default_build_info);
...
OPCUAServer.prototype._on_CreateSessionRequest = function (message, channel) {
    ...
    if (server.currentSessionCount >= server.maxAllowedSessionNumber) {
        ...
        var response = new CreateSessionResponse({ responseHeader: { serviceResult: StatusCodes.BadTooManySessions   } });
    ...
    if (!request.clientNonce ||  request.clientNonce.length<32) {
        if (channel.securityMode !== endpoints_service.MessageSecurityMode.NONE) {
            ...
            var response = new CreateSessionResponse({ responseHeader: { serviceResult: StatusCodes.BadNonceInvalid   } });
            return channel.send_response("MSG", response, message);
        }
    }
