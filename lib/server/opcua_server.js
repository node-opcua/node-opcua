...
var package = JSON.parse(fs.readFileSync(package_json_file));
...
function OPCUAServer(options) {
    ...
    self.userManager = options.userManager || { isValidUser: function(/*userName,password*/){ return false; }};
...
OPCUAServer.prototype.isValidUserNameIdentityToken = function(channel,session,userTokenPolicy,userIdentityToken)
{
    ...
    // assert(serverPrivateKey instanceof Buffer);
    // serverPrivateKey = crypto_utils.toPem(serverPrivateKey, "RSA PRIVATE KEY");
...
OPCUAServer.prototype._on_ActivateSessionRequest = function (message, channel) {
    ...
    if (!server.isValidUserIdentityToken(channel,session,request.userIdentityToken)) {
        return rejectConnection(StatusCodes.BadIdentityTokenRejected);
    }
