...
ServerSession.prototype.__defineGetter__("addressSpace",function() { return this.parent.address_space; });
...
ServerSession.prototype._createSessionObjectInAddressSpace = function() {
    ...
    if (!self.addressSpace) {
        console.log("ServerSession#_createSessionObjectInAddressSpace : no address_space");
        return ; // no addressSpace
    }
    var root = self.addressSpace.findObject(makeNodeId(ObjectIds.RootFolder));
    ...
    // self.addressSpace.findObject(makeNodeId(ObjectIds.Server_ServerDiagnostics));
    var serverDiagnostics =  root.objects.server.serverDiagnostics;

     if (!serverDiagnostics.sessionsDiagnosticsSummary) {
        console.log("ServerSession#_createSessionObjectInAddressSpace : no serverDiagnostics.sessionsDiagnosticsSummary");
        return false;
    }

    self.sessionObject = self.addressSpace.addObject({
        nodeId: self.nodeId,
...
//    var serverDiagnostics = addressSpace.findObject("ServerDiagnostics");
