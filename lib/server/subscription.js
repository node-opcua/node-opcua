...
Subscription.prototype._attempt_to_publish_notification = function () {
    ...
    if (publishEngine.pendingPublishRequestCount > 0) {
        ...
        if (self.state !== SubscriptionState.CLOSED) {
            assert(notificationMessage.notificationData.length >0, "We are not expectinga keep-alive message here");
        ...
        if (self.state !== SubscriptionState.LATE) {
            // only notify first time we get to LATE state
        ...
        debugLog("subscription " + self.id + " set to LATE");
    }
...
Subscription.prototype.process_subscription = function () {
    ...
    if (doDebug) {
        debugLog("process_subscription hasPendingNotifications=",self.hasPendingNotifications," publishingEnabled=",self.publishingEnabled);
    }
...
Subscription.prototype._initialTick = function () {
    var self = this;
    // send initial tick
    setImmediate(function () {
        debugLog("performing initial subscription notification");
        assert(self.publishIntervalCount === 0);
...
Subscription.prototype._tick = function () {
    ...
    // istanbul ignore next
    if (doDebug) {
        debugLog(" processing subscription", self.id, self.publishIntervalCount);
    }
    ...
    if (self.lifeTimeHasExpired) {
        ...
        var publishEngine = self.publishEngine;
        if (publishEngine.pendingPublishRequestCount === 0 && self.hasPendingNotifications) {

            // istanbul ignore next
            if (doDebug) {
                debugLog("subscription set to LATE ");
                debugLog(" publishEngine.pendingPublishRequestCount = ",publishEngine.pendingPublishRequestCount);
                debugLog(" subscription.hasPendingNotifications     = ",self.hasPendingNotifications);
            }
            self.state = SubscriptionState.LATE;
            return;
        }
...
Subscription.prototype.resetKeepAliveCounter = function () {
    ...
    // istanbul ignore next
    if (doDebug) {
        debugLog("Resetting keepAliveCounter = ",self._keep_alive_counter,self.maxKeepAliveCount);
    }
};
...
Subscription.prototype.increaseKeepAliveCounter = function () {
    ...
    // istanbul ignore next
    if (doDebug) {
        debugLog("Increasing keepAliveCounter = ", self._keep_alive_counter, self.maxKeepAliveCount);
    }
};
...
Subscription.prototype.collectNotificationData = function () {
    ...
    /**
     * extract up to maxNotificationsPerPublish notifications into a Dequeue
     * @param monitoredItems
     * @param maxNotificationsPerPublish
     * @return {Deque|exports|module.exports}
     */
