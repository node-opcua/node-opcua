...
Subscription.prototype.adjustSamplingInterval = function (samplingInterval, node) {
    ...
    if (samplingInterval < 0) {
    ...
    } else if (samplingInterval === 0 || samplingInterval < MonitoredItem.minimumSamplingInterval) {
        ...
        samplingInterval = samplingInterval === 0 ? 0 : MonitoredItem.minimumSamplingInterval;
...
function __validateDataChangeFilter(filter) {
...
function __validateFilter(filter, itemToMonitor) {
    ...
    if (filter && filter instanceof subscription_service.EventFilter && itemToMonitor.attributeId !== AttributeIds.EventNotifier) {
        ...
        return StatusCodes.BadMonitoredItemFilterInvalid;
    }
    ...
    if (filter instanceof subscription_service.DataChangeFilter) {
        return __validateDataChangeFilter(filter);
    }
...
Subscription.prototype.createMonitoredItem = function (address_space, timestampsToReturn, monitoredItemCreateRequest) {
    ...
    var statusCodeFilter = __validateFilter(filter, itemToMonitor);
    ...
    var monitoredItemCreateResult = self.createMonitoredItem1(timestampsToReturn, monitoredItemCreateRequest, node);
 ...
 */
Subscription.prototype.createMonitoredItem1 = function (timestampsToReturn, monitoredItemCreateRequest, node) {

    var self = this;

    // note : most of the parameter inconsistencies shall have been handled by the caller
    // any error here will raise an assert
...
Subscription.prototype.prepareNotification = function () {
    ...
    // Only collect data change notification for the
