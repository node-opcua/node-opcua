...
var defaultPublishingInterval = 250;
...
Subscription.prototype._attempt_to_publish_notification = function () {
    ...
    if (publishEngine.pendingPublishRequestCount > 0) {
        ...
        if (self.state === SubscriptionState.CLOSED) {
            debugLog("Subscription has been closed , subscriptionId=",self.id);
            return;
        }
...
Subscription.prototype.process_subscription = function () {
    ...
    if (doDebug) {
        debugLog("Subscription#process_subscription id =", self.id," hasPendingNotifications=",self.hasPendingNotifications," publishingEnabled=",self.publishingEnabled);
    }
...
Subscription.prototype._tick = function () {
    ...
    // istanbul ignore next
    if (doDebug) {
        debugLog("   Subscription#_tick  processing subscriptionId=", self.id, " publishingIntervalCount =",self.publishIntervalCount);
    }
...
Subscription.prototype.collectNotificationData = function () {
    ...
    for (i=0;i<n;i++) {
        ...
        all_notifications = all_notifications.addEach(notifications);
    }
