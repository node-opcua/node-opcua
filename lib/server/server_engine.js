...
ServerEngine.prototype.initialize = function (options, callback) {
    ...
    generate_address_space(self.addressSpace, options.nodeset_filename, function () {
        ...
        function findObjectNodeId(name) {
            var obj = self.addressSpace.findObject(name);
        ...
        self.rootFolder = self.addressSpace.findObject("RootFolder");
        ...
        function bindVariableIfPresent(nodeId, opts) {
            ...
            var obj = self.addressSpace.findObject(nodeId);
            if (obj) {
        ...
        function bindServerDiagnostics() {
            ...
            var serverDiagnosticsSummary = self.addressSpace.findObject(makeNodeId(VariableIds.Server_ServerDiagnostics_ServerDiagnosticsSummary));
        ...
        function bindServerStatus() {
            ...
            var serverStatus = self.addressSpace.findObject(makeNodeId(VariableIds.Server_ServerStatus));
            if(serverStatus) {
                serverStatus.bindExtensionObject();
                serverStatus.miminumSamplingInterval = 250;
            }

            var currentTimeNode = self.addressSpace.findObject(makeNodeId(VariableIds.Server_ServerStatus_CurrentTime));
 ...
 * @return {NodeId}
 */
ServerEngine.prototype.resolveNodeId = _DEPRECATED_SOON_REMOVED("ServerEngine#resolveNodeId",function (nodeId) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.resolveNodeId(nodeId);
},"!!");

ServerEngine.prototype.addFolder = _DEPRECATED_SOON_REMOVED("ServerEngine#addFolder",function (parentFolder, options) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.addFolder(parentFolder, options);
}," use serverEngine.addressSpace.addFolder(...) instead");
 ...
 * @return {BaseNode}
 */
ServerEngine.prototype.findObject = _DEPRECATED_SOON_REMOVED("ServerEngine#findObject",function (nodeId) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.findObject(nodeId);
},"");
 ...
 *
 */
ServerEngine.prototype.addVariable = _DEPRECATED_SOON_REMOVED("ServerEngine#addVariable",function (options) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.addVariable(options);
},"");
 ...
 * @return {Object}
 */
ServerEngine.prototype.addMethod = _DEPRECATED_SOON_REMOVED("ServerEngine#addMethod",function (parentObject, options) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.addMethod(parentObject, options);
},"");

ServerEngine.prototype.addObjectInFolder = _DEPRECATED_SOON_REMOVED("ServerEngine#addObjectInFolder",function (parentObject, options) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.addObjectInFolder(parentObject, options);
},"");
ServerEngine.prototype.addView = _DEPRECATED_SOON_REMOVED("ServerEngine#addView",function (options) {
    var self = this;
    return self.addressSpace.addView(options);
},"");
ServerEngine.prototype.__findObject = function (nodeId) {
    ...
    assert(nodeId instanceof NodeId);
    return this.addressSpace.findObject(nodeId);
};
...
ServerEngine.prototype.browseSingleNode = function (nodeId, browseDescription) {
    ...
    if (typeof nodeId === "string") {
        var node = self.addressSpace.findObject(self.addressSpace.resolveNodeId(nodeId));
        if (node) {
            nodeId = node.nodeId;
        }
    }
    ...
    if (browseDescription.referenceTypeId instanceof NodeId) {
        if (browseDescription.referenceTypeId.value === 0) {
            ...
            var rf = self.addressSpace.findObject(browseDescription.referenceTypeId);
...
ServerEngine.prototype.bindVariable = function (nodeId, options) {
    ...
    var obj = this.addressSpace.findObject(nodeId);
...
ServerEngine.prototype.__internal_bindMethod = function (nodeId, func) {
    ...
    var methodNode = self.addressSpace.findObject(nodeId);
    if (!methodNode) {
        return;
    }
...
ServerEngine.prototype.refreshValues = function (nodesToRefresh, callback) {
    ...
    var objs = _.map(nodesToRefresh, function (n) {
        return self.addressSpace.findObject(n.nodeId);
    });
