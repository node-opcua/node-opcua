...
ServerEngine.prototype.initialize = function (options, callback) {
    ...
    generate_address_space(self.addressSpace, options.nodeset_filename, function () {
        ...
        function bindVariableIfPresent(nodeId, opts) {
            ...
            var obj = self.findObject(nodeId);
            if (obj) {
        ...
        function bindServerDiagnostics() {
            ...
            var serverDiagnosticsSummary = self.findObject(makeNodeId(VariableIds.Server_ServerDiagnostics_ServerDiagnosticsSummary));
        ...
        function bindServerStatus() {
            ...
            var serverStatus = self.findObject(makeNodeId(VariableIds.Server_ServerStatus));
            if(serverStatus) {
                serverStatus.bindExtensionObject();
                serverStatus.miminumSamplingInterval = 250;
            }

            var currentTimeNode = self.findObject(makeNodeId(VariableIds.Server_ServerStatus_CurrentTime));
        ...
        function prepareServerDiagnostics() {
            ...
            var serverObject = self.findObjectByBrowseName("Server");
            ...
            var serverDiagnostics = serverObject.getComponentByName("ServerDiagnostics");
...
ServerEngine.prototype._build_new_NodeId = function () {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace._build_new_NodeId();
...
ServerEngine.prototype.getFolder = function (folder) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.getFolder(folder);
};
...
ServerEngine.prototype.findObjectByBrowseName = function (browseName) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
    return self.addressSpace.findObjectByBrowseName(browseName);
};
 ...
 *
 */
ServerEngine.prototype.addVariable = function (options) {
    var self = this;
    assert(self.addressSpace instanceof AddressSpace); // initialize not called
...
ServerEngine.prototype.addView = function (options) {
    var self = this;
...
ServerEngine.prototype.browseSingleNode = function (nodeId, browseDescription) {
    ...
    if (typeof nodeId === "string") {
        var node = self.addressSpace.findObjectByBrowseName(nodeId);
        if (node) {
            nodeId = node.nodeId;
        }
    }
