...
/**
 * @returns {UInt32} the approximate number of seconds until the server will be shut down. The
...
//xx        var ServerStateDataTypeId = self.address_space.findDataType("ServerState").nodeId;
        ...
        bindStandardScalar(VariableIds.Server_ServerStatus_SecondsTillShutdown,
            DataType.UInt32,function() { return self.secondsTillShutdown(); });

        bindStandardScalar(VariableIds.Server_ServerStatus_ShutdownReason,
            DataType.LocalizedText,function() { return new LocalizedText({text: self.shutdownReason}); });
...
ServerEngine.prototype._readSingleNode = function (nodeToRead,timestampsToReturn) {
    ...
    if (!obj) {
        ...
        //xx console.log("XXXX nodeID",nodeId.displayText(),"attributeId",attributeId);
        ...
        //Xx console.log("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".yellow,obj.browseName,dataValue.toString());
        //xx var b = new require("../misc/binaryStream").BinaryStream();        dataValue.encode(b);
