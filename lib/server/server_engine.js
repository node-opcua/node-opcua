...
ServerEngine.prototype.__defineGetter__("serverNameURN",function(){
    return "urn:" + this.serverName;
});

ServerEngine.prototype.__defineGetter__("serverNamespaceName",function(){
    return "urn:" + this.serverName;
});
...
ServerEngine.prototype.initialize = function (options, callback) {
    ...
    // opc.tcp://PCETIENNE-PC:53530/OPCUA/SimulationServer
    ...
    generate_address_space(self.address_space, options.nodeset_filename, function () {
            ...
            get: function () {
                return new Variant({
                    ...
                    value: [
                        ...
                        self.serverNamespaceName, // namespace 1 is our
                    ]
                });
            },
            set: null // read only
        ...
        bindStandardScalar(VariableIds.Server_ServerDiagnostics_ServerDiagnosticsSummary_SessionAbort,
            DataType.UInt32, function () {
                return 0; // not supported yet
            });
...
ServerEngine.prototype.addMethod = function(parentObject,options) {
    ...
    var _inputArgs = new Variant({
        dataType: DataType.ExtensionObject,
        arrayType: VariantArrayType.Array,
        value: options.inputArguments.map(function(options){ return new Argument(options)})
    });
    ...
    var inputArguments = self.addVariable(method,{
        ...
        value: { get: function() { return _inputArgs; }}
    });

    var _ouputArgs = new Variant({
        dataType: DataType.ExtensionObject,
        arrayType: VariantArrayType.Array,
        value: options.outputArguments.map(function(options){ return new Argument(options)})
    });
    ...
    var outputArguments = self.addVariable(method,{
        ...
        value: { get: function() { return _ouputArgs; }}
    });
...
ServerEngine.prototype.createSession = function (options) {
    ...
    var session = new ServerSession(self._session_counter,sessionTimeout);
    ...
    session.on("timeout", function () {
        ...
        console.log("closing SESSION because of timeout = ".cyan,session.sessionTimeout, " has expired without a keep alive".cyan);
...
ServerEngine.prototype.getSession = function (authenticationToken) {
    ...
    if( typeof authenticationToken === "string") {
        return self._sessions[authenticationToken];
    }
