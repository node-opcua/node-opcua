...
ServerSidePublishEngine.prototype.process_subscriptionAcknowledgements = function (subscriptionAcknowledgements) {
    ...
    var results = subscriptionAcknowledgements.map(subscriptionAcknowledgement => {
...
ServerSidePublishEngine.prototype._on_PublishRequest = function (request, callback) {
    ...
    var publishData = {request, results: subscriptionAckResults,callback};
...
ServerSidePublishEngine.prototype.send_response_for_request = (publishData, response) => {
...
ServerSidePublishEngine.prototype.__defineGetter__("currentMonitoredItemsCount", function () {
    ...
    var result =  _.reduce(this._subscriptions,(cumul, subscription) => cumul + subscription.monitoredItemCount,0);
...
ServerSidePublishEngine.prototype.findSubscriptionWaitingForFirstPublish = function(){
    ...
    var subscriptions_waiting_for_first_reply = _.filter(this._subscriptions, subscription => !subscription.messageSent && subscription.state === SubscriptionState.LATE);
...
ServerSidePublishEngine.prototype.findLateSubscriptionSortedByPriority= function () {
    ...
    // istanbul ignore next
    if (false) {
        console.log(late_subscriptions.map(s => "[ id = " + s.id +
            " prio=" + s.priority +
...
ServerSidePublishEngine.transferSubscriptions = (srcPublishEngine, destPublishEngine) => {
