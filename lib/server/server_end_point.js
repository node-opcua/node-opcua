...
function OPCUAServerEndPoint(options) {
    ...
    self._server.on("connection", function (socket) {
        ...
        //xx console.log('server has established a connection with  : ' + socket.remoteAddress + ':' + socket.remotePort);
    }).on("close", function () {
...
function _makeEndpointDescription(options) {
    ...
    assert(options.securityPolicy);// instanceof SecurityPolicy
    ...
    // ressource Path is a string added at the end of the url such as "/UA/Server"
    var ressourcePath = options.ressourcePath ||"";
    ...
    var endpoint = new EndpointDescription({
        ...
        endpointUrl: "opc.tcp://" + hostname + ":" + path.join(""+options.port, ressourcePath),
...
OPCUAServerEndPoint.prototype.addStandardEndpointDescriptions = function(options) {
    ...
    if (crypto_utils.isFullySupported()) {
        ...
        for (var i = 0; i < options.securityModes.length; i++) {
            ...
            if (securityMode === MessageSecurityMode.NONE) { continue; }

            for (var j = 0; j < options.securityPolicies.length; j++) {
                var securityPolicy = options.securityPolicies[j];
                if (securityPolicy === SecurityPolicy.None) { continue; }
