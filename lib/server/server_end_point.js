...
OPCUAServerEndPoint.prototype.endpointDescriptions = function () {
    ...
    if (!self._endpoints) {
        ...
        // install endpoint with 2048 bits cryptology (not yet supported)
        var cert256 = self.getCertificate256();
        if (cert256) {
            self._endpoints.push(_makeEndpointDescription(port, cert256, s.MessageSecurityMode.SIGN, SecurityPolicy.Basic256));
            self._endpoints.push(_makeEndpointDescription(port, cert256, s.MessageSecurityMode.SIGNANDENCRYPT, SecurityPolicy.Basic256));
//xx            self._endpoints.push(_makeEndpointDescription(port, cert256, s.MessageSecurityMode.SIGN, SecurityPolicy.Basic256Rsa15));
//xx            self._endpoints.push(_makeEndpointDescription(port, cert256, s.MessageSecurityMode.SIGNANDENCRYPT, SecurityPolicy.Basic256Rsa15));
        }
    }
