...
MonitoredItem.prototype._stop_timer = function () {
    ...
    if (self._attribute_changed_callback) {
        ...
        //xx console.log("xxxx unsetting event name :",event_name);
        ...
        assert(self.samplingInterval === 0);
...
MonitoredItem.prototype._start_timer = function () {
    ...
    if (self.samplingInterval === 0) {
        ...
        // initiate first read (this requires self._sampli  ngId to be set)
...
MonitoredItem.prototype._on_sampling_timer = function () {
    ...
    // todo: MonitoringMode.Sampling must be dealt with
    assert(self.monitoringMode !== MonitoringMode.Sampling," TO DO : unsupported yet");

    if (self.monitoringMode !== MonitoringMode.Reporting) {
        console.log(" ->FixME! monitoringMode is not Reporting");
        return;
    }

    if (self._samplingId) {
        //xx console.log("xxxx ON SAMPLING");
        assert(!self._is_sampling);
...
MonitoredItem.prototype.__defineGetter__("isSampling",function() {
    ...
    return self._is_sampling ||
        _.isFunction(self._value_changed_callback) ||
