const { coerceBoolean, coerceByteString, coerceUInt32, coerceInt32, coerceUInt64 } = require("..");
...
describe("ByteString", () => {
    ...
    it("coerceByteString with base64 string", () => {
        coerceByteString(Buffer.from([1, 2, 3]).toString("base64")).toString("hex").should.eql("010203");
    })
    it("coerceByteString with a Buffer", () => {
        coerceByteString(Buffer.from([1, 2, 3])).toString("hex").should.eql("010203");
    })
});
