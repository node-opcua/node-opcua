...
| Endpoints / EndpointsExclude + Add/RemoveEndpoint | §4.4.1, §4.4.9-10 | ✅ | restriction store + endpoint matching (default-field rules) + AddEndpoint/RemoveEndpoint Methods bound + client + audited, and **enforced at role resolution**: `SessionContext` now surfaces the endpoint URL (`getEndpointUrl` on the session), so `getCurrentUserRoles` filters by endpoint; integration-tested end-to-end |
...
| UserManagement audit (AuditUpdateMethodEventType) | §5.2 | ✅ | AddUser/ModifyUser/RemoveUser/ChangePassword raise an audit event on the Server object — **password arguments are never included** (redacted message, no inputArguments); unit-tested (incl. no-leak assertions) + integration-tested |
