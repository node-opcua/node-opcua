...
describe("testing NodeIds", function () {
    ...
    it("NodeId#toString with addressSpace object (findNode) 2", () => {
        ...
        nodeId.toString({ addressSpace }).should.eql("ns=3;s=AAA Hello");
...
describe("#sameNodeId", function () {
    ...
    it("nodeId with special characters", () => {
        const nodeId1 = coerceNodeId("ns=6;s=::a:b:Hello#World");
        const nodeId2 = coerceNodeId("ns=6;s=::a:b:Hello#World#ByeBye");
        sameNodeId(nodeId1, nodeId2).should.eql(false);
        nodeId1.toString().should.not.eql(nodeId2.toString());
    });
});
