...
describe("Testing PermissionFlag", function () {
    it("should create a permission level flags from a string", () => {
        ...
        makePermissionFlag("All").should.eql(allPermissions);
    });
    ...
    it("permissionFlagToString", () => {
        permissionFlagToString(allPermissions).should.eql(
            "Browse | ReadRolePermissions | WriteAttribute | WriteRolePermissions | WriteHistorizing | Read | Write | ReadHistory | InsertHistory | ModifyHistory | DeleteHistory | ReceiveEvents | Call | AddReference | RemoveReference | DeleteNode | AddNode"
        );
    });
});
