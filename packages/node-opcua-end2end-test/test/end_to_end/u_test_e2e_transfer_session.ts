...
async function perform(client1: OPCUAClient | ClientSession, request: Request) {
    try {
        const response = await promisify((client1 as any).performMessageTransaction).call(client1, request);
        return response;
    } catch (err) {
...
const _createSession = async (client1: OPCUAClient) =>
    await promisify((client1 as any)._createSession).call(client1);

const _activateSession = async (
    client1: OPCUAClient, 
    session1: ClientSession, 
    userIdentityInfo: UserIdentityInfo) =>
    await promisify((client1 as any)._activateSession).call(client1, session1, userIdentityInfo);
        ...
        it("RQB08 - server should raise an error if a session is reactivated with different user identity tokens", async () => {
            ...
            try {
                ...
                try {
                    ...
                    // Scrap
                    (session1 as any).userIdentityInfo = { type: UserTokenType.Anonymous };
