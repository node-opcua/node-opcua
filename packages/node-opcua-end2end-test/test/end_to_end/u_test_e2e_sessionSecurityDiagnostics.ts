...
export function t(test: any) {
    describe("SDS2 Testing SessionSecurityDiagnostics", () => {
        ...
        it("SDS2-A server should expose a ServerSecurityDiagnostic object", async () => {
            ...
            await client.withSessionAsync(
                ...
                async (session) => {
                    ...
                    const _sessionDiagnostics = dataValues[0].value.value;
            ...
            await client.withSessionAsync(connectionPoint, async (session) => {
                const nodesToRead = [
                    {
        ...
        it("SDS2-B server should expose a SessionSecurityDiagnostics per Session", async () => {
            ...
            await perform_operation_on_subscription_async(client, connectionPoint, async (session, _subscription) => {
                const nodeToRead = {
                    nodeId: session.sessionId,
                    attributeId: AttributeIds.BrowseName
                };
                const _dataValue = await session.read(nodeToRead);
                ...
                const _browseResult = await session.browse([browseDesc]);
        ...
        async function count_number_of_exposed_sessionDiagnostics() /*: Promise<number>*/ {
            ...
            return await perform_operation_on_subscription_async(client, connectionPoint, async (session, _subscription) => {
                ...
                // enumerate all sessions available
                //xx console.log(browseResult[0].toString());
                _sessionDiagnosticsNodeId = browseResult[0].references?.[0].nodeId;
        ...
        it("SDS2-D server should remove SessionSecurityDiagnostic when session is closed", async () => {
            ...
            //  createSession
            const client = OPCUAClient.create(clientOptions);
            await perform_operation_on_subscription_async(client, connectionPoint, async (_session, _subscription) => {
        ...
        it("SDS2-F it should not be possible to read sessionSecurityDiagnostics with a secure connection and non admin user", async () => {
            const client = OPCUAClient.create(clientOptions);
            const anonymous_connectionPoint: EndpointWithUserIdentity = {
            ...
            await perform_operation_on_subscription_async(client, anonymous_connectionPoint, async (session, _subscription) => {
                const nodesToRead = [
                    {
                        nodeId: makeNodeId(VariableIds.Server_ServerDiagnostics_SessionsDiagnosticsSummary_SessionDiagnosticsArray),
                        attributeId: AttributeIds.Value
