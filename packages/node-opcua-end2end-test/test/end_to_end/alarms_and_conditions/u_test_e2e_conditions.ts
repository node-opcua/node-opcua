...
export function t(test: any) {
    describe("A&C monitoring conditions", () => {
        ...
        function dump_field_values(fields: string[], values: Variant[]) {
            fields
                .map((field, index) => [field, values[index]] as [string, Variant])
                .forEach(([field, value]) => {
                    let str = "";
        ...
        function extract_value_for_field(fieldName: string, result: Variant[]): Variant {
            ...
            should(index).be.greaterThanOrEqual(
                0,
            ...
            async function wait_until_refresh_end() {
                ...
                await new Promise<void>((resolve, reject) => {
                    if (test.spy_monitored_item1_changes.callCount === 3) {
                        stepInfo(
                            chalk.red(`spy_monitored_item1_changes.callCount = ${test.spy_monitored_item1_changes.callCount}`)
                        );
                        resolve();
                    }
            ...
            await perform_operation_on_subscription(client, test.endpointUrl, async (session, subscription) => {
                ...
                stepInfo("then when server raises a new condition_event");
                {
                    ...
                    await waitUntilCondition(
                        ...
                        1000,
                        " wait for condition raiseNewCondition to be detected"
                    );
                }
        ...
        describe("test on Disabled conditions", () => {
            /*
        ...
        async function perform_test_with_condition(
            ...
            await perform_operation_on_subscription(client, test.endpointUrl, async (session, subscription) => {
                ...
                //  we_should_verify_than_branch_one_is_no_longer_here
                {
                    ...
                    const _dataValues0 = test.spy_monitored_item1_changes.getCall(0).args[0];
                    const _dataValues1 = test.spy_monitored_item1_changes.getCall(1).args[0];
                    const _dataValues2 = test.spy_monitored_item1_changes.getCall(2).args[0];
