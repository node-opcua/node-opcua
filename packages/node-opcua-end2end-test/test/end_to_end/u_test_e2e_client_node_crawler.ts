...
export function t(test: any) {
    describe("NodeCrawler", () => {
        ...
        it("CRAWL1- should crawl for a complete tree", async () => {
            await redirectToFileAsync("NodeCrawler_complete_tree.log", async () => {
        ...
        it("CRAWL2- should crawl for a complete tree with limited node per browse and read request", async () => {
            await perform_operation_on_client_session(client, endpointUrl, async (session) => {
                ...
                let browsed_node1 = 0;
                let browsed_node2 = 0;
                crawler1.on("browsed", (_nodeElement, _data) => {
                    browsed_node1++;
                });
                crawler2.on("browsed", (_nodeElement, _data) => {
                    browsed_node2++;
                });
        ...
        it("CRAWL5- should display a tree", async () => {
            await redirectToFileAsync("crawler_display_tree.log", async () => {
                await perform_operation_on_client_session(client, endpointUrl, async (the_session) => {
                    ...
                    const _obj = await crawler.read(nodeId);
                    crawler.dispose();
                });
            });
        });
