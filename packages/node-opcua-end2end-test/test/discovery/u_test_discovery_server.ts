...
export function t(test: TestHarness) {
    ...
    describe("DISCO2 - DiscoveryServer2", function (this: Mocha.Runnable) {
        ...
        it("DISCO2-0 registered server shutting down ( and unregistering) before discovery server does should not cause issues", async () => {
            ...
            // 2. When the registered server is shutdown before the discovery server
            await server.shutdown();

            // 3. Then the discovery server should not encounter issues when it eventually shuts down
            await discoveryServer.shutdown();
            await pause(1000);
