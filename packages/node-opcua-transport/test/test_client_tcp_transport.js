...
describe("testing ClientTCP_transport", function() {
    ...
    it("should close the socket and emit a close event when disconnect() is called", function(done) {
        ...
        transport.on("close", function(err) {
            transport_confirms_that_close_event_has_been_processed.should.eql(
                false,
        ...
        transport.connect(
            ...
            function(err) {
                if (err) {
                    ...
                    const regexp_4 = /Invalid endpoint url/; // error from pareseEndpointUrl()
                    ...
                    (test1 || test2 || test3 || test4).should.eql(true, "expecting one of those error message");
                    done();
                } else {
                    done(new Error("Should have raised a connection error"));
                }
