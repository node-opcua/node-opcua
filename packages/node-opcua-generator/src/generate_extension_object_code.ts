...
function writeEnumeratedType(enumeratedType: EnumeratedType) {
    ...
    let arrayValues = enumeratedType.enumeratedValues
...
export async function generate(filename: string, generatedTypescriptFilename: string) {
    ...
    try {
        ...
        write(`import {`);
        write(`      StructuredTypeSchema, registerEnumeration, buildStructuredType,`);
        write(`       BaseUAObject, initialize_field, initialize_field_array,`);
        write(`       check_options_correctness_against_schema, `);
        write(`      registerClassDefinition, parameters `);
        write(`} from "node-opcua-factory";`);



        write(`import {`);
        write(`    NodeId, ExpandedNodeId, makeExpandedNodeId, NodeIdLike`);
        write(`} from "node-opcua-nodeid";`);

        write(`import {`);
        write(`       Variant, encodeVariant, decodeVariant, VariantLike`);
        write(`} from "node-opcua-variant";`);

        write(`import {`);
        write(`       DataValue, encodeDataValue, decodeDataValue, DataValueLike, `);
        write(`       TimestampsToReturn, encodeTimestampsToReturn, decodeTimestampsToReturn, _enumerationTimestampsToReturn`);
        write(`} from "node-opcua-data-value";`);

        write(`import {`);
        write(`     ExtensionObject, encodeExtensionObject, decodeExtensionObject`);
        write(`} from "node-opcua-extension-object";`);

        write(`import {`);
        write(`     LocalizedText, encodeLocalizedText, decodeLocalizedText, LocalizedTextLike,`);
        write(`     QualifiedName, encodeQualifiedName, decodeQualifiedName, QualifiedNameLike,`);
        write(`     DiagnosticInfo, encodeDiagnosticInfo, decodeDiagnosticInfo,`);
        write(`     BrowseDirection, encodeBrowseDirection, decodeBrowseDirection, _enumerationBrowseDirection`);
        write(`} from "node-opcua-data-model";`);


        write(`import {`);
        write(`     StatusCode, encodeStatusCode, decodeStatusCode`);
        write(`} from "node-opcua-status-code";`);

        write(`import {`);
        write(`    NumericRange, encodeNumericRange, decodeNumericRange`);
        write(`} from "node-opcua-numeric-range";`);
        // xx write(`import { MessageSecurityMode } from "node-opcua-secure-channel";`);

        write(`import {`);
        write(`    Byte, SByte, Double, Float,`);
        write(`    Int8, Int16, Int32, Int64,`);
        write(`    UInt8, UInt16, UInt32, UInt64,`);
        write(`    encodeByte, decodeByte,`);
        write(`    encodeDouble, decodeDouble,`);
        write(`    encodeFloat, decodeFloat,`);
        write(`    encodeInt8, decodeInt8,`);
        write(`    encodeInt16, decodeInt16,`);
        write(`    encodeInt32, decodeInt32,`);
        write(`    encodeInt64, decodeInt64,`);
        write(`    encodeUInt8, decodeUInt8,`);
        write(`    encodeUInt16, decodeUInt16,`);
        write(`    encodeUInt32, decodeUInt32,`);
        write(`    encodeUInt64, decodeUInt64,`);
        write(`    encodeString, decodeString,`);
        write(`    encodeBoolean, decodeBoolean,`);
        write(`    encodeByteString, decodeByteString,`);
        write(`    encodeNodeId, decodeNodeId,`);
        write(`    encodeExpandedNodeId, decodeExpandedNodeId,`);
        write(`    DateTime, encodeDateTime, decodeDateTime,`);
        write(`    encodeUAString, decodeUAString,`);
        write(`    encodeUABoolean, decodeUABoolean,`);
        write(`    CharArray,`);
        write(`    UAString,`);
        write(`    UABoolean,`);
        write(`    ByteString,`);
        write(`    Guid, encodeGuid, decodeGuid,`);
        write(`    encodeArray, decodeArray,`);
        write(`} from "node-opcua-basic-types";`);
