...
function analyzeCycles(cycles: Cycle[]): void {
    ...
    for (let i = 0; i < cycles.length; i++) {
        ...
        const packageDependencyCounts = cycle.packages.map((pkg) => ({
            package: pkg,
            count: cycle.packages.filter((other) => other !== pkg && cycle.packages.includes(other)).length
        }));

        const mostDependent = packageDependencyCounts.reduce((max, current) => (current.count > max.count ? current : max));
