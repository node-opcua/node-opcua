...
function isExternalPackage(moduleName: string): boolean {
    ...
    const globalTestModules = ["should", "sinon", "mocha"];
...
function main(): void {
    ...
    for (const pkg of packages) {
        ...
        const extraneousDependencies = findExtraneousDependencies(
            Array.from(sourceImports),
            Array.from(testImports),
            dependencies,
            devDependencies
        ...
        if (missingDetailed.length > 0) {
            ...
            // Group by section
            const bySection = missingDetailed.reduce(
                (acc, dep) => {
        ...
        if (extraneousDependencies.length > 0) {
            ...
            const bySection = extraneousDependencies.reduce(
                (acc, dep) => {
                    if (!acc[dep.section]) {
                        acc[dep.section] = [];
                    }
